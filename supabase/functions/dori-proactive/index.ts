...
function localNow(tz: string): {
  ...
  return {
    date: new Date(`${dayKey}T${parts.hour}:${parts.minute}:00`),
    hour,
    minute,
    dayKey,
...
async function morningBrief(supabase: SupabaseClient, ctx: UserCtx) {
  ...
  // Pull today's data
  const startOfDay = new Date(ctx.nowLocal);
  startOfDay.setHours(0, 0, 0, 0);
  const endOfDay = new Date(ctx.nowLocal);
  ...
  if (tasks.length > 0) {
    ...
    tasks
      .slice(0, 3)
      .forEach((t: { title: string; priority: string }) =>
...
async function meetingPrep(supabase: SupabaseClient, ctx: UserCtx) {
  ...
  for (const e of events || []) {
    ...
    const startStr = new Date(e.start_time).toLocaleTimeString("en-GB", {
      timeZone: ctx.tz,
      hour: "2-digit",
      minute: "2-digit",
    });
...
async function emailActionItems(supabase: SupabaseClient, ctx: UserCtx) {
  ...
  const list = emails
    .map(
      (
        e: { from_name?: string; from_email?: string; subject?: string; snippet?: string },
        i: number,
      ) =>
  ...
  let parsed: {
    items?: Array<{ index: number; action_type: string; action_summary: string; urgency?: string }>;
  };
  try {
          ...
          items: {
            ...
            items: {
              ...
              properties: {
                ...
                action_summary: {
                  type: "string",
                  description: "One short sentence describing what needs to be done.",
                },
  ...
  const taskItems = items
    .filter((it) => it.action_type === "todo" || it.action_type === "payment")
    .slice(0, 5);
...
Deno.serve(async (req) => {
  ...
  const { data: users, error } = await supabase
    ...
    .select(
      `user_id, enabled, prefer_voice_replies, timezone, morning_briefing_time, evening_review_time,
    ...
    .eq("enabled", true)
    .eq("telegram_proactive_enabled", true);
  ...
  for (const s of users || []) {
    try {
      ...
      const ctx: UserCtx = {
        ...
        settings: s,
        preferVoice: !!s.prefer_voice_replies,
        tz,
        nowLocal: now.date,
        todayKey: now.dayKey,
