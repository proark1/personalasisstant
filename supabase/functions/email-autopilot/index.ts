...
serve(async (req) => {
  ...
  try {
    ...
    (links || []).forEach((l: { user_id: string; chat_id?: number | null }) => {
      if (l.chat_id) chatIdByUser.set(l.user_id, Number(l.chat_id));
    });

    const users = (rawUsers || []).map((u: { user_id: string; [k: string]: unknown }) => ({
      ...u,
      telegram_chat_id: chatIdByUser.get(u.user_id) ?? null,
    }));
    ...
    for (const u of (users as ProactiveSettings[]) || []) {
      ...
      const { data: actionEmails } = await sb
        .from("user_emails")
        .select(
          "id, from_name, from_email, subject, ai_summary, ai_suggested_action, priority_score",
        )
        .eq("user_id", u.user_id)
        .gte("received_at", sinceIso)
        .eq("user_archived", false)
