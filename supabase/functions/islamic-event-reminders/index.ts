...
const ISLAMIC_EVENTS: IslamicEvent[] = [
  ...
  },
  {
    ...
    hijriDate: "27 Rajab",
    description:
      "The miraculous Night Journey from Makkah to Jerusalem and ascension to the heavens.",
    type: "major",
  },
  {
  ...
  },
  {
    ...
    description:
      "The Night of Power - better than 1000 months. Worship this night equals 83+ years of worship.",
    type: "major",
  },
  {
    name: "Eid al-Fitr",
...
Deno.serve(async (req) => {
  try {
    ...
    if (upcomingEvents.length === 0) {
      return new Response(JSON.stringify({ message: "No events within reminder window" }), {
        status: 200,
      });
    }
    ...
    if (!users || users.length === 0) {
      return new Response(JSON.stringify({ message: "No users with event reminders enabled" }), {
        status: 200,
      });
    }
        ...
        batch.map(async (userSettings) => {
          try {
            for (const { event: upcomingEvent, date: eventDate } of upcomingEvents) {
              ...
              const emoji =
                upcomingEvent.type === "major"
                  ? "⭐"
                  : upcomingEvent.type === "fasting"
                    ? "🌙"
                    : "✨";
...
`.trim();
    ...
    console.log(
      `Event reminders processed: ${sentCount} sent, ${failedCount} failed for ${upcomingEvents.length} events`,
    );
    return new Response(
      JSON.stringify({
        success: true,
