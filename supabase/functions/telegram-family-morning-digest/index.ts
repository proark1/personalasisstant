...
Deno.serve(async (req) => {
  ...
  const { data: groups, error } = await supabase
    ...
    .select(
      "id, chat_id, owner_user_id, partner_user_id, morning_digest_enabled, morning_digest_hour, morning_digest_last_sent_on, voice_digest_enabled, title",
    )
    .eq("is_active", true);
  ...
  for (const g of groups || []) {
    if (!g.morning_digest_enabled && !force) {
      skipped++;
      continue;
    }
    ...
    if (!force) {
      if (localHour !== (g.morning_digest_hour ?? 7)) {
        skipped++;
        continue;
      }
      if (g.morning_digest_last_sent_on === localDate) {
        skipped++;
        continue;
      }
    }
