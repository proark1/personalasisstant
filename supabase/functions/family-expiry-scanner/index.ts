...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const { data: docs } = await supabase
      ...
      .select(
        "id, user_id, document_type, expiry_date, reminder_days_before, family_member_id, last_reminded_at",
      );
    ...
    for (const d of docs || []) {
      ...
      if (
        d.last_reminded_at &&
        today.getTime() - new Date(d.last_reminded_at).getTime() < 7 * 86400000
      )
        continue;
    ...
    for (const p of pets || []) {
      if (
        p.last_reminded_at &&
        today.getTime() - new Date(p.last_reminded_at).getTime() < 7 * 86400000
      )
        continue;
      const candidates = [
    ...
    for (const m of maint || []) {
      ...
      if (
        m.last_reminded_at &&
        today.getTime() - new Date(m.last_reminded_at).getTime() < 7 * 86400000
      )
        continue;
      await supabase.from("user_notifications").insert({
        user_id: m.user_id,
        type: "family_expiry",
        title: daysLeft < 0 ? `${m.task_name} is OVERDUE` : `${m.task_name} due in ${daysLeft}d`,
    ...
    const { data: vehicles } = await supabase
      ...
      .select(
        "id, user_id, nickname, next_inspection_date, insurance_renewal_date, next_service_date, next_tire_change_date, last_reminded_at",
      );
    for (const v of vehicles || []) {
      if (
        v.last_reminded_at &&
        today.getTime() - new Date(v.last_reminded_at).getTime() < 7 * 86400000
      )
        continue;
      const items = [
    ...
    for (const t of trads || []) {
      const daysLeft = Math.floor(
        (new Date(t.next_occurrence!).getTime() - today.getTime()) / 86400000,
      );
      if (daysLeft < 0 || daysLeft > 3) continue;
      if (
        t.last_reminded_at &&
        today.getTime() - new Date(t.last_reminded_at).getTime() < 2 * 86400000
      )
        continue;
      await supabase.from("user_notifications").insert({
        user_id: t.user_id,
