...
serve(async (req) => {
  ...
  try {
    const { tasks, focusMinutes, completionRate, checkins, weekStartDate }: ReviewRequest = await req.json();
    ...
    const moodScores: Record<string, number> = { 'great': 5, 'good': 4, 'okay': 3, 'stressed': 2, 'tired': 1 };
    const avgMood = checkins.length > 0 
      ? checkins.reduce((sum, c) => sum + (moodScores[c.mood || 'okay'] || 3), 0) / checkins.length
      : 3;
...
${completedTasks.slice(0, 5).map(t => `- ${t.title} (${t.category}, ${t.priority} priority)`).join('\n')}
