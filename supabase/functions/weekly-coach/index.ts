  ...
  try {
    ...
    const averageEnergy =
      energyCheckins.length > 0
        ? energyCheckins.reduce((acc, c) => acc + (energyMap[c.energy_level] || 3), 0) /
    ...
    const productivityScore = Math.round(
      taskCompletionRate * 0.4 + focusScore * 0.3 + habitCompletionRate * 0.3,
    );
    const wellbeingScore = Math.round(
      ((averageMood || 3) / 5) * 40 +
        ((averageSleep || 7) / 8) * 30 +
        ((averageEnergy || 3) / 5) * 30,
    );
    ...
    const summaryData = {
      ...
      goals: goals
        .slice(0, 5)
        .map((g) => ({ name: g.name, progress: (g.current_value / g.target_value) * 100 })),
...
- recommendations: Array of 3 specific, actionable recommendations for next week`,
    ...
    const report = {
      ...
      goal_progress: goals.slice(0, 5).reduce(
        (acc, g) => {
