  ...
  try {
    ...
    if (
      ...
      if (!connection.refresh_token) {
        await admin
          .from("external_calendar_connections")
          .update({ last_sync_error: "missing_refresh_token" })
          .eq("id", connectionId);
        return new Response(
          JSON.stringify({ error: "Failed to refresh Outlook token. Please reconnect." }),
          {
            status: 401,
            headers: { ...corsHeaders, "Content-Type": "application/json" },
          },
      ...
      if (!newTokens) {
        await admin
          .from("external_calendar_connections")
          .update({ last_sync_error: "token_refresh_failed" })
          .eq("id", connectionId);
        return new Response(
          JSON.stringify({ error: "Failed to refresh Outlook token. Please reconnect." }),
          {
      ...
      await admin
        ...
        .update({
          ...
          refresh_token: await encryptTokenIfConfigured(
            newTokens.refresh_token || connection.refresh_token,
          ),
    ...
    if (!evResp.ok) {
      ...
      await admin
        .from("external_calendar_connections")
        .update({ last_sync_error: errText.slice(0, 500) })
        .eq("id", connectionId);
