...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const [checkinsRes, eventsRes, tasksRes] = await Promise.all([
      supabase
        .from("daily_checkins")
        .select("checkin_date, mood, energy_level, sleep_hours, stress_level, day_rating")
        .eq("user_id", userId)
        .gte("checkin_date", past.slice(0, 10))
          ...
          messages: [
            {
              role: "system",
              content:
                "You are an energy-aware scheduling coach. Combine recent energy/mood/sleep with calendar density to give ONE concise (2-3 sentence) suggestion for today. Identify low-energy patterns and suggest moving deep work to higher-energy days. Be encouraging, never preachy.",
            },
            {
              role: "user",
              content: `Today: ${now.toISOString().slice(0, 10)}\n\nLast 7 days check-ins:\n${JSON.stringify(checkinsRes.data ?? [])}\n\nCalendar density next 7 days (date → meeting count):\n${JSON.stringify(density)}\n\nOpen high-priority tasks:\n${JSON.stringify((tasksRes.data ?? []).filter((t) => t.priority === "high"))}`,
            },
          ],
        }),
      },
    ...
    return new Response(
      JSON.stringify({ suggestion, density, checkins: checkinsRes.data?.length ?? 0 }),
      {
