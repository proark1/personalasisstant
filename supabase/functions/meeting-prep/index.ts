          ...
          messages: [
            {
              role: "system",
              content:
                'You are Dori, a concise personal assistant. Generate a 1-2 sentence "what to remember" prep note for an upcoming meeting. Be warm, specific, actionable. No fluff.',
            },
            {
              role: "user",
...
serve(async (req) => {
  ...
  for (const ev of (events || []) as EventRow[]) {
    ...
    if (settings?.quiet_hours_enabled) {
      ...
      const inQuiet = startH > endH ? hour >= startH || hour < endH : hour >= startH && hour < endH;
      if (inQuiet) continue;
    }
    ...
    const title = bucket === 1 ? `🔔 Starting now: ${ev.title}` : `📅 ${ev.title} in ${bucket} min`;

    const message = [ev.location ? `📍 ${ev.location}` : null, prepNote || null]
      .filter(Boolean)
      .join("\n");
