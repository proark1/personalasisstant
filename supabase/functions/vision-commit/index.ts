...
serve(async (req) => {
  ...
  try {
    ...
    if (cap.status !== "extracted")
      return json({ error: `cannot commit while ${cap.status}` }, 409);
    ...
    try {
      switch (kind) {
        case "receipt": {
          ...
          const { data: tx, error: tErr } = await admin
            ...
            .insert({
              ...
              metadata: {
                capture_id: cap.id,
                line_items: (merged as Record<string, unknown>).line_items ?? [],
              },
            })
            .select("id")
            .single();
          ...
          await recordCreateUndo(
            "financial_transactions",
            tx.id,
            `${merchant ? `${merchant} · ` : ""}${total ?? "?"} ${currency}`,
            "transaction",
          );
        ...
        case "unknown": {
          const title =
            strOrNull(merged.title) || (kind === "whiteboard" ? "Whiteboard scan" : "Scanned text");
          const bulletsBlock = Array.isArray((merged as Record<string, unknown>).bullets)
            ? "\n\n" +
              ((merged as Record<string, unknown>).bullets as string[])
                .map((b) => `- ${b}`)
                .join("\n")
            : "";
