  ...
  try {
    ...
    if (checkins && checkins.length >= 7) {
      ...
      if (sleepProductivityData.length >= 5) {
        ...
        if (goodSleepDays.length >= 3 && badSleepDays.length >= 2) {
          const goodSleepAvgRating = goodSleepDays.reduce((acc, d) => acc + d.rating, 0) / goodSleepDays.length;
          const badSleepAvgRating = badSleepDays.reduce((acc, d) => acc + d.rating, 0) / badSleepDays.length;
    ...
    if (checkins && tasks) {
      ...
      if (highEnergyDays.length >= 2 && lowEnergyDays.length >= 2) {
        const highEnergyAvg = highEnergyDays.reduce((acc, d) => acc + d.tasksCompleted, 0) / highEnergyDays.length;
        const lowEnergyAvg = lowEnergyDays.reduce((acc, d) => acc + d.tasksCompleted, 0) / lowEnergyDays.length;
    ...
    if (focusSessions && focusSessions.length >= 5) {
      ...
      if (completionRate < 0.5 && focusSessions.length >= 8) {
        const avgDuration = focusSessions.reduce((acc, s) => acc + s.duration_minutes, 0) / focusSessions.length;
        patterns.push({
    ...
    if (!existingSummary && checkins && checkins.length >= 3) {
      ...
      const weekHabits = habitLogs?.filter(l => {
      ...
      await supabase
        ...
        .insert({
          ...
          focus_minutes: weekFocus.reduce((acc, s) => acc + (s.is_completed ? s.duration_minutes : 0), 0),
          habits_completed: weekHabits.length,
          avg_mood: weekCheckins.length > 0 
            ? weekCheckins.filter(c => c.mood).reduce((acc, c) => acc + getMoodValue(c.mood), 0) / weekCheckins.filter(c => c.mood).length
            : null,
          avg_energy: weekCheckins.length > 0
            ? weekCheckins.filter(c => c.energy_level).reduce((acc, c) => acc + getEnergyValue(c.energy_level), 0) / weekCheckins.filter(c => c.energy_level).length
            : null,
          avg_sleep_hours: weekCheckins.length > 0
            ? weekCheckins.filter(c => c.sleep_hours).reduce((acc, c) => acc + c.sleep_hours, 0) / weekCheckins.filter(c => c.sleep_hours).length
            : null,
