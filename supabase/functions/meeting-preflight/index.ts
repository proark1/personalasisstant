        ...
        messages: [
          {
            role: "system",
            content:
              "Generate a sharp 30-second pre-meeting brief. 3-5 bullets max. Include: who you're meeting, last interaction summary, open threads, key context. Be concise and actionable.",
          },
          {
            role: "user",
            content: `Meeting: "${event.title}" at ${event.start_time}\nLocation: ${event.location || "N/A"}\nDescription: ${event.description || "none"}\n\nRelevant context:\n${JSON.stringify(ctx)}`,
          },
        ],
      }),
    },
  ...
  try {
    ...
    for (const ev of todo) {
      ...
      const [contactsRes, emailsRes, contractsRes, profileRes] = await Promise.all([
        supabase
          .from("user_contacts")
          .select("id, name, last_contacted_at, notes")
          .eq("user_id", ev.user_id)
          .limit(5),
        supabase
          .from("user_emails")
          .select("id, subject, from_name, snippet")
          .eq("user_id", ev.user_id)
      ...
      const matched = (contactsRes.data ?? []).filter((c) =>
        attendeeNames.some(
          (n: string) => c.name.toLowerCase().includes(n) || n.includes(c.name.toLowerCase()),
        ),
      );

      const ctx = {
        matched_contacts: matched,
        recent_emails: emailsRes.data ?? [],
      ...
      if (error) {
        console.error("brief insert err", error);
        continue;
      }
