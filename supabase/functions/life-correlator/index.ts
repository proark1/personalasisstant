...
serve(async (req) => {
  ...
  try {
    ...
    const [checkinsResult, tasksResult, focusResult, habitsResult, eventsResult, expensesResult] =
      await Promise.all([
        supabase
          .from("daily_checkins")
          .select("*")
    ...
    console.log(
      `Data fetched - Checkins: ${checkins.length}, Tasks: ${tasks.length}, Focus: ${focusSessions.length}, Habits: ${habitLogs.length}, Events: ${events.length}`,
    );
    ...
    if (checkins.length >= 10) {
      ...
      const tasksByDate = groupByDate(
        tasks.filter((t) => t.completed && t.completed_at),
        "completed_at",
      );
      ...
      if (sleepData.length >= 7) {
        ...
        const correlation =
          denomSleep && denomTasks ? numerator / Math.sqrt(denomSleep * denomTasks) : 0;
    ...
    if (checkins.length >= 7 && events.length >= 5) {
      ...
      if (stressData.length >= 5) {
        ...
        const correlation =
          denomEvents && denomStress ? numerator / Math.sqrt(denomEvents * denomStress) : 0;
    ...
    if (habitLogs.length >= 10 && focusSessions.length >= 5) {
      ...
      const focusByDate = groupByDate(
        focusSessions.filter((f) => f.is_completed),
        "started_at",
      );
      ...
      const dataPoints = dates
        .map((date) => ({
          ...
          focusMinutes: (focusByDate.get(date) || []).reduce(
      ...
      if (dataPoints.length >= 7) {
        ...
        const correlation =
          denomHabits && denomFocus ? numerator / Math.sqrt(denomHabits * denomFocus) : 0;
    ...
    if (expenses.length >= 10 && checkins.length >= 7) {
      ...
      if (moodData.length >= 5) {
        ...
        if (highSpendDays.length >= 3 && normalDays.length >= 5) {
          const highSpendMood =
            highSpendDays.reduce((a, b) => a + b.mood, 0) / highSpendDays.length;
    ...
    for (const corr of correlations) {
      ...
      if (existing) {
        ...
        await supabase.from("life_correlations").insert({
          user_id: userId,
