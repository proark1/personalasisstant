...
const corsHeaders = {
  ...
  "Access-Control-Allow-Headers":
    "authorization, x-client-info, apikey, content-type, x-meetingbot-signature, x-signature",
};
  ...
  try {
    ...
    if (analysis && typeof analysis === "object") {
      ...
      if (Array.isArray(analysis.action_items))
        patch.action_items = analysis.action_items.slice(0, 50);
    ...
    const events = Array.isArray((row.metadata as Record<string, unknown>)?.events)
      ? ((row.metadata as Record<string, unknown>).events as Record<string, unknown>[])
      : [];
    ...
    if (event === "bot.done" && Array.isArray(patch.action_items as unknown)) {
      if (!row.tasks_created_at) {
        ...
        const inserts = items
          .map((it) =>
            typeof it?.task === "string" && it.task.trim().length > 0
              ? {
                  user_id: row.user_id,
...
function sanitiseTranscript(arr: unknown[]): TranscriptEntry[] {
  return arr.slice(0, 5000).map((e) => {
    ...
    return {
      speaker:
        typeof entry?.speaker === "string" ? (entry.speaker as string).slice(0, 120) : "Unknown",
