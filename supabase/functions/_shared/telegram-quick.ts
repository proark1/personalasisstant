...
export function buildBestNextActionMessage(args: {
  ...
  const msUntilNextEvent = nextEvent ? new Date(nextEvent.start_time).getTime() - now.getTime() : null;
  const nextEventIsSoon = msUntilNextEvent !== null && msUntilNextEvent >= 0 && msUntilNextEvent <= 45 * 60 * 1000;
  const parts: string[] = [workspaceId ? '<b>🎯 Best next move — workspace</b>' : '<b>🎯 Best next move</b>'];
  if (pendingCount > 0) {
    parts.push(`You have <b>${pendingCount}</b> pending approval${pendingCount === 1 ? '' : 's'}. Clear those first so Dori can finish queued work.`);
    parts.push('Send <code>/approvals</code> to approve or cancel them.');
  } else if (nextEvent && nextEventIsSoon) {
    parts.push(`Prep for <b>${escapeTelegramHtml(truncateTelegramText(nextEvent.title, 120))}</b> at ${escapeTelegramHtml(formatTelegramDate(nextEvent.start_time, timezone))}.`);
    parts.push('Ask me: <code>prep me for this meeting</code>.');
  } else if (overdue) {
    parts.push(`Tackle overdue task: <b>${escapeTelegramHtml(truncateTelegramText(overdue.title, 120))}</b>.`);
    parts.push('If it no longer matters, reply: <code>delete that task</code>. If it should move, reply: <code>move it to tomorrow</code>.');
  } else if (dueToday) {
    parts.push(`Do today’s task: <b>${escapeTelegramHtml(truncateTelegramText(dueToday.title, 120))}</b>.`);
    parts.push('When done, reply: <code>done</code> or <code>mark that complete</code>.');
  } else if (nextEvent) {
    parts.push(`Your next event is <b>${escapeTelegramHtml(truncateTelegramText(nextEvent.title, 120))}</b> at ${escapeTelegramHtml(formatTelegramDate(nextEvent.start_time, timezone))}.`);
    parts.push('Use the open block before then for your highest-energy work.');
  } else {
    parts.push('Nothing urgent is visible. Use this as a planning block: send <code>plan my day</code> or capture anything on your mind by voice note.');
  }
...
export function buildMemorySnapshotMessage(args: {
  ...
  if (memoryRows.length === 0 && prefRows.length === 0) {
    parts.push('I do not have saved facts or learned preferences yet. Tell me “remember that …” and I’ll store useful context.');
  }
  if (memoryRows.length > 0) {
    ...
    memoryRows.forEach((m) => {
      ...
      parts.push(`• <b>${escapeTelegramHtml(label)}</b>${escapeTelegramHtml(category)}: ${escapeTelegramHtml(value)}`);
    });
  }
  if (prefRows.length > 0) {
    parts.push('\n<b>Learned preferences</b>');
    prefRows.forEach((pref) => {
      const confidence = typeof pref.confidence === 'number' ? ` (${Math.round(pref.confidence * 100)}%)` : '';
      ...
      parts.push(`• <b>${escapeTelegramHtml(key)}</b>: ${escapeTelegramHtml(value)}${confidence}${seen}`);
    });
  }
