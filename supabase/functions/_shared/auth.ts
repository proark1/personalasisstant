...
export async function resolveUserId(req: Request): Promise<ResolvedAuth | null> {
  ...
  if (serviceKey && token === serviceKey && tgUserId) {
    ...
    // If INTERNAL_AUTH_SECRET is configured (it should be in prod),
    // require a valid HMAC token. This means a leaked service key alone
    // is not enough — the attacker also needs the rotating signing key.
    // Backwards-compatible: if INTERNAL_AUTH_SECRET is unset, fall back
    // to legacy behaviour and log a warning so the gap is visible.
    ...
    if (internalSecret) {
      ...
      console.warn('[auth] INTERNAL_AUTH_SECRET unset — service-role + x-telegram-user-id path is unprotected');
    }
