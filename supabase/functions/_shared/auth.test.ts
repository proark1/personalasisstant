...
function setupEnv(opts: { secret?: boolean } = { secret: true }) {
  ...
  Deno.env.delete("ALLOW_LEGACY_INTERNAL_AUTH");
...
Deno.test("rejects service-role delegation when INTERNAL_AUTH_SECRET is unset", async () => {
...
Deno.test("legacy path: accepts only when the explicit escape hatch is set", async () => {
  setupEnv({ secret: false });
  Deno.env.set("ALLOW_LEGACY_INTERNAL_AUTH", "true");
  const r = await resolveUserId(
    req({
      Authorization: `Bearer ${SERVICE_KEY}`,
      "x-telegram-user-id": USER_ID,
