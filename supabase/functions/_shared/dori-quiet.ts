...
export async function isUserQuietNow(
    ...
    if (needSettings) {
      lookups.push(
        supabase.from('proactive_settings')
          .select('enabled, quiet_hours_enabled, quiet_hours_start, quiet_hours_end, focus_mode_until, suppress_during_events')
          .eq('user_id', userId).maybeSingle(),
      );
    }
    const results = await Promise.all(lookups);
    let idx = 0;
    if (needTz) { tz = results[idx++]?.data?.timezone || undefined; }
    if (needSettings) { settings = results[idx++]?.data || null; }
  }
  ...
  if (respectEvents && settings.suppress_during_events !== false) {
    ...
    const { data: live } = await supabase.from('events')
      .select('end_time').eq('user_id', userId)
      .lte('start_time', nowIso).gte('end_time', nowIso).limit(1).maybeSingle();
    if (live) {
      return { quiet: true, reason: 'in_event', resumeAt: live.end_time };
    }
  }
