...
const taskFields = {
  ...
  query: {
    type: "string",
    description:
      "Title fragment to fuzzy-match the task when you do not have an id (update/delete/complete only). For UPDATE, always use query for identification — title is the new value. For delete/complete with no id, query (preferred) or title may be used as the search key.",
  },
};
...
export const NATIVE_TOOLS: ToolDef[] = [
  {
    ...
    function: {
      name: "manage_task",
      description:
        "Create, update, complete, or delete a task. Always set dueDate when the user mentions any time reference.",
      parameters: {
        type: "object",
        properties: {
  ...
  },
  {
    ...
    function: {
      name: "schedule_event",
      description:
        "Create a calendar event. Use this for new events; use manage_event to update or delete existing ones.",
      parameters: {
        type: "object",
  ...
  },
  {
    ...
    function: {
      name: "find_time",
      description:
        "Workspace mode only. Returns 3-5 free slots when all named participants are available.",
      parameters: {
        type: "object",
        properties: {
          query: {
    ...
    function: {
      ...
      parameters: {
        ...
        properties: {
          action: {
            type: "string",
            enum: ["create", "update", "delete", "mark_contacted", "search"],
          },
          contact: {
            type: "object",
            properties: {
  ...
  },
  {
    ...
    function: {
      ...
      description:
        'Add, remove, or clear items on the family shopping list. Use action="remove" with a name to delete a single item (fuzzy match), or action="clear" to empty all unchecked items.',
      parameters: {
        type: "object",
        properties: {
          action: {
  ...
  },
  {
    ...
    function: {
      name: "send_email",
      description:
        "Send an email via the user connected Gmail account. Always shown to the user for confirmation first.",
      parameters: {
        type: "object",
        properties: {
          email: {
  ...
  },
  {
    ...
    function: {
      name: "web_search",
      description:
        "Search the web for real-time / factual / news information outside the user personal data.",
      parameters: {
        type: "object",
  ...
  },
  {
    ...
    function: {
      name: "save_memory",
      description:
        "Persist a long-term fact / preference / pattern about the user. Silent — do not announce.",
      parameters: {
        type: "object",
        properties: {
          memory: {
            ...
            properties: {
              type: {
                type: "string",
                enum: ["preference", "fact", "pattern", "goal", "milestone"],
              },
  ...
  },
  {
    ...
    function: {
      name: "log_wellbeing",
      description:
        "Log mood, energy, sleep hours, water, exercise, stress for today. Send only fields the user mentioned.",
      parameters: {
        type: "object",
        properties: {
          wellbeing: {
  ...
  },
  {
    ...
    function: {
      name: "manage_goal",
      description:
        "Create, update progress on (use add or current_value), list, or delete a long-term goal.",
      parameters: {
        type: "object",
        properties: {
  ...
  },
  {
    ...
    function: {
      ...
      description:
        "Shift many open tasks at once. Use filter.when (today|overdue|tomorrow) or filter.date, plus shift_days.",
      parameters: {
        type: "object",
        properties: {
          bulk: {
            ...
            properties: {
              filter: {
                type: "object",
                properties: {
                  when: { type: "string", enum: ["today", "overdue", "tomorrow"] },
                  date: { type: "string" },
                },
              },
  ...
  },
  {
    ...
    function: {
      name: "append_note",
      description:
        "Append content to an existing note found by title fragment (does not create a new note).",
      parameters: {
        type: "object",
        properties: {
          note: {
  ...
  // ============= Phase 2: 19 new tools =============
  {
    ...
    function: {
      name: "task_filter",
      description:
        'List tasks with filters (status, priority, tag, due range). Use for "show my high-priority tasks", "what is tagged work".',
      parameters: {
        type: "object",
        properties: {
          data: {
            ...
            properties: {
              ...
              tag: { type: "string" },
              when: { type: "string", enum: ["today", "tomorrow", "overdue", "week"] },
            },
          },
        },
        required: ["data"],
  ...
  },
  {
    ...
    function: {
      ...
      parameters: {
        type: "object",
        properties: { data: { type: "object", properties: { limit: { type: "number" } } } },
        required: ["data"],
      },
    },
  },
  {
    ...
    function: {
      name: "email_action",
      description:
        "Perform an action on an email: star, unstar, archive, trash, snooze, mark_read, mark_unread, forward, unsubscribe.",
      parameters: {
        type: "object",
        properties: {
          data: {
            ...
            properties: {
              ...
              query: { type: "string", description: "Subject fragment if no messageId" },
              action: {
                type: "string",
                enum: [
                  "star",
              ...
              snoozeUntil: { type: "string", description: "ISO datetime for snooze" },
      ...
      parameters: {
        ...
        properties: {
          data: {
            type: "object",
            properties: {
              action: { type: "string", enum: ["add", "remove", "list", "low"] },
              name: { type: "string" },
              quantity: { type: "number" },
              unit: { type: "string" },
      ...
      parameters: {
        ...
        properties: {
          data: {
            type: "object",
            properties: {
              action: { type: "string", enum: ["log", "schedule", "list"] },
              name: { type: "string" },
              dose: { type: "string" },
              time: { type: "string" },
            },
  ...
  },
  {
    ...
    function: {
      ...
      parameters: {
        type: "object",
        properties: {
          data: { type: "object", properties: { city: { type: "string" } }, required: ["city"] },
        },
        required: ["data"],
      },
    },
...
export function toolCallsToLegacyXml(
