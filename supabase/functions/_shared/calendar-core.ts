...
function pushAllowed(conn: CalendarConnection): boolean {
  ...
  return (
    !conn.sync_direction ||
    conn.sync_direction === "two_way" ||
    conn.sync_direction === "one_way_push"
  );
}
...
function pullAllowed(conn: CalendarConnection): boolean {
  return (
    !conn.sync_direction ||
    conn.sync_direction === "two_way" ||
    conn.sync_direction === "one_way_pull"
  );
}
...
<c:calendar-query xmlns:d="DAV:" xmlns:c="urn:ietf:params:xml:ns:caldav">
  ...
  <c:filter><c:comp-filter name="VCALENDAR"><c:comp-filter name="VEVENT">
    ...
    if (!resp.ok && resp.status !== 207) {
      ...
      const dataBlocks = [...xml.matchAll(/<calendar-data[^>]*>([\s\S]*?)<\/calendar-data>/gi)].map(
        (m) => m[1].trim(),
      );
