...
} from "../_shared/telegram-digest.ts";
...
async function handlePlansCommand(supabase: SupabaseClient, chatId: number, userId: string | null) {
  ...
  const { data: plans, error } = await supabase
    ...
    .select(
      "id, title, status, completed_step_count, step_count, current_step_idx, blocks:dori_plan_steps(idx,title,status)",
    )
    .eq("user_id", userId)
  ...
  if (!plans || plans.length === 0) {
    await tgSend(
      chatId,
      '📋 No active plans. Ask Dori to plan something multi-step ("plan my Dubai trip", "set up a weekly review") and it\'ll appear here.',
    );
    return;
  }
  ...
  for (const p of plans as Array<{
    ...
    step_count: number;
    status: string;
    blocks: Array<{ idx: number; title: string; status: string }>;
  }>) {
...
async function getHouseholdMembers(
  ...
  (profiles || []).forEach(
    (p: { user_id: string; display_name?: string | null; email?: string | null }) =>
      map.set(p.user_id, p.display_name || p.email?.split("@")[0] || "Member"),
  );
  return {
    ids: idList,
    nameOf: (uid: string) => map.get(uid) || "Member",
    multi: idList.length > 1,
  };
}
...
async function getWorkspaceMembers(
  ...
  const fallback =
    rows.find((m) => m.role === "owner") || rows.find((m) => m.role === "admin") || rows[0] || null;
  const map = new Map<string, string>();
...
async function handleStandup(
  ...
  const memberIds = (members as Array<{ user_id: string; display_name?: string | null }>).map(
    (m) => m.user_id,
  );
  const [{ data: yesterdayDone }, { data: todayTasks }, { data: upcomingEvents }] =
    await Promise.all([
      supabase
        .from("tasks")
  ...
  for (const ev of upcomingEvents || []) {
    ...
    if (m) {
      const when = new Date(ev.start_time).toLocaleTimeString("en-GB", {
        timeZone: tz,
        hour: "2-digit",
        minute: "2-digit",
      });
  ...
  for (const m of members as Array<{ user_id: string; display_name?: string | null }>) {
    ...
    if (entry.done.length === 0 && entry.today.length === 0 && entry.events.length === 0) {
      ...
      if (entry.done.length)
        block.push(
          `✅ <i>Yesterday:</i> ${entry.done.slice(0, 5).join(" · ")}${entry.done.length > 5 ? ` (+${entry.done.length - 5})` : ""}`,
        );
      if (entry.today.length)
        block.push(
          `📌 <i>Today:</i> ${entry.today.slice(0, 5).join(" · ")}${entry.today.length > 5 ? ` (+${entry.today.length - 5})` : ""}`,
        );
      if (entry.events.length)
        block.push(`📅 <i>Scheduled:</i> ${entry.events.slice(0, 3).join(" · ")}`);
    }
...
async function handleUpcomingMeetings(
  supabase: SupabaseClient,
  ids: string[],
  household: HouseholdInfo,
  tz?: string,
  ...
  const upcoming = (events || []).filter(
    (e: { title: string; start_time: string; location?: string | null; user_id: string }) => {
  ...
  for (const list of lists) {
    ...
    if (items?.length)
      items.forEach((i: { name: string; quantity: number }) =>
        out.push(`• ${i.quantity > 1 ? `${i.quantity}× ` : ""}${i.name}`),
      );
    else out.push("  (empty)");
  }
  return out.join("\n");
  ...
  for (const list of lists) {
    ...
    for (const item of items) {
      if (totalSent >= MAX_TAPPABLE) {
        await tgSend(
          chatId,
          `… and ${items.length - totalSent} more. Use the app for the full list.`,
        );
        break;
      }
...
async function sendTappableAgenda(
  ...
  const label =
    dayOffset === 0
      ...
      : dayOffset === 1
        ? "Tomorrow"
        : start.toLocaleDateString("en-GB", { weekday: "long", day: "2-digit", month: "short" });
  ...
  await tgSend(
    chatId,
    `<b>📅 ${label}</b> — ${events?.length || 0} events, ${tasks?.length || 0} open tasks. Tap a card to act on it.`,
  );
  let sent = 0;
  const MAX_CARDS = 12;
  ...
  if (totalItems > sent) {
    await tgSend(
      chatId,
      `… and ${totalItems - sent} more. Ask me about specific items for details.`,
    );
  }
  return true;
}
...
async function handleBirthdays(
  ...
  const upcoming = (
    data as Array<{ occurs_on: string; user_id: string; user_contacts?: { name?: string } | null }>
  )
    .map((d) => {
  ...
  upcoming.forEach((b) => {
    ...
    const when =
      b.days === 0
        ...
        : b.days === 1
          ? "tomorrow"
          : `in ${b.days}d (${b.next.toLocaleDateString("en-GB", { day: "2-digit", month: "short" })})`;
...
async function _handleContracts(
  supabase: SupabaseClient,
  ids: string[],
  ...
  let q = supabase
    .from("contracts")
    .select("name, provider, cost_amount, cost_frequency, renewal_date, end_date, user_id")
    .in("user_id", ids)
    .eq("is_active", true);
  if (expiringOnly) {
    const in60 = new Date();
    ...
    q = q.or(
      `renewal_date.lte.${in60.toISOString().slice(0, 10)},end_date.lte.${in60.toISOString().slice(0, 10)}`,
    );
  }
  const { data } = await q.order("renewal_date", { nullsFirst: false }).limit(20);
  if (!data?.length)
    return expiringOnly ? "✅ Nothing expiring in the next 60 days." : "📄 No active contracts.";
  ...
  (
    ...
    const cost = c.cost_amount
      ? ` — ${c.cost_amount}€${c.cost_frequency ? "/" + c.cost_frequency : ""}`
      : "";
...
async function handleProperties(supabase: SupabaseClient, ids: string[]): Promise<string> {
  const { data } = await supabase
    .from("properties")
    .select("name, property_type, city, country, current_value")
    .in("user_id", ids)
    .eq("is_active", true);
...
async function handleVehicles(supabase: SupabaseClient, ids: string[]): Promise<string> {
  const { data } = await supabase
    .from("vehicles")
    .select("name, make, model, year, license_plate, next_service_date, insurance_renewal")
    .in("user_id", ids);
  ...
  (
    ...
    if (v.next_service_date)
      upcoming.push(`service ${new Date(v.next_service_date).toLocaleDateString("en-GB")}`);
    if (v.insurance_renewal)
      upcoming.push(`insurance ${new Date(v.insurance_renewal).toLocaleDateString("en-GB")}`);
    lines.push(
      `• ${v.name}${desc ? ` — ${desc}` : ""}${plate}${upcoming.length ? `\n  ↳ ${upcoming.join(", ")}` : ""}`,
    );
  });
  return lines.join("\n");
}
...
async function handleHealth(
  ...
  for (const uid of ids) {
    ...
    const latest = new Map<
      string,
      { metric_type: string; value: number | string; unit?: string }
    >();
    (
      data as Array<{
        metric_type: string;
      ...
      if (!latest.has(m.metric_type)) latest.set(m.metric_type, m);
    });
    const prefix = household.multi ? `<b>${household.nameOf(uid)}</b> — ` : "";
    const bits = Array.from(latest.values())
...
async function handleEmailActions(
  ...
  (
    ...
    const subj =
      ((e.suggested_payload as Record<string, unknown>)?.subject as string | undefined) ||
      ((e.suggested_payload as Record<string, unknown>)?.from as string | undefined) ||
      e.suggested_action ||
      e.category;
...
async function handleOverdue(
  ...
  (
    data as Array<{ title: string; due_date: string; priority?: string | null; user_id: string }>
  ).forEach((t) => {
    const pr = t.priority === "high" ? "🔴" : t.priority === "low" ? "⚪️" : "🟡";
...
async function handleNotesSearch(
  supabase: SupabaseClient,
  ids: string[],
  query: string,
): Promise<string> {
  ...
  (data as Array<{ title?: string | null; content?: string | null }>).forEach((n) => {
    const snippet = String(n.content || "")
      .replace(/\s+/g, " ")
      .slice(0, 120);
    lines.push(
      `• <b>${escapeHtml(n.title || "Untitled")}</b>${snippet ? ` — ${escapeHtml(snippet)}…` : ""}`,
    );
  });
  return lines.join("\n");
}
...
async function handleFreeTime(
  ...
  for (const [k, v] of Object.entries(dayMap)) {
    if (hint.includes(k)) {
      ...
      ranked = ranked.filter(
        (s: { start: string; local: string }) => new Date(s.start).getDay() === target,
      );
      break;
    }
  }
  ...
  if (!ranked.length)
    return `😕 No free ${durationMinutes}-minute slots${dayHint ? ` for "${dayHint}"` : " in the next 7 days"}.`;
  const lines = [
    `<b>🗓 Free ${durationMinutes}-min slots</b>${dayHint ? ` (${escapeHtml(dayHint)})` : ""}`,
  ];
  ranked.forEach((s: { start: string; local: string }, i: number) =>
    lines.push(`${i + 1}. ${s.local}`),
  );
  return lines.join("\n");
}
...
async function handleEmailDraft(
  ...
  try {
    ...
    if (!draft)
      return `⚠️ Couldn't draft a reply right now. Open the email in the app to draft it manually.`;
...
async function handlePrayers(supabase: SupabaseClient, userId: string): Promise<string> {
  ...
  try {
    ...
    console.error("prayers fetch failed", e);
  }
...
async function handleQibla(
  ...
  try {
    ...
    const qRes = await fetch(`https://api.aladhan.com/v1/qibla/${lat}/${lng}`).then((r) =>
      r.json(),
    );
...
async function handleQuran(supabase: SupabaseClient, userId: string, arg: string): Promise<string> {
  const q = arg.trim();
  if (!q)
    return "Usage: <code>/quran &lt;surah name or number&gt;</code> (e.g. /quran al-fatiha or /quran 1)";
  void supabase;
  void userId;
  try {
    ...
    if (surahNum == null)
      return `🕌 I don't recognise "${q}". Try a number 1-114 or a name like /quran al-fatiha.`;
    const r = await fetch(
    ...
    const arabic = j.data[0];
    const english = j.data[1];
...
async function handleChores(supabase: SupabaseClient, ids: string[]): Promise<string> {
  ...
  const lines = (data as Array<{ title: string; due_date?: string | null }>)
    .slice(0, 10)
    .map((t) => {
...
Deno.serve(async (req) => {
  ...
  if (!group && !workspace_id) {
    await tgSend(
      chat_id,
      "🔒 This group is not linked to Dori yet. Use /linkfamily for a family space or /linkworkspace for a workspace.",
    );
  ...
  if (workspace_id) {
    ...
    household = await getHouseholdMembers(
      supabase,
      group!.owner_user_id,
  ...
  if (!userForChat) {
    await tgSend(
      chat_id,
      "🔒 I can see this group, but no workspace member is linked yet. Link your Telegram account in private chat, then try again.",
    );
    return new Response('{"ok":true}', { headers: corsHeaders });
  }
  ...
  if (lower === "/me") {
    await tgSend(
      chat_id,
      await handleMeDigest(supabase, userForChat, workspace_id || null, userTimezone),
    );
    return new Response('{"ok":true}', { headers: corsHeaders });
  }
  ...
  if (lower === "/standup") {
    if (!workspace_id) {
      await tgSend(
        chat_id,
        "🧑‍🤝‍🧑 /standup works in workspace-linked groups. Try /linkworkspace <code> first.",
      );
    } else {
  ...
  if (lower.startsWith("/comment")) {
    ...
    if (!rest) {
      await tgSend(
        chat_id,
        "Usage: <code>/comment &lt;task title&gt; :: &lt;your comment&gt;</code>",
      );
      return new Response('{"ok":true}', { headers: corsHeaders });
    }
    ...
    if (matches.length > 1) {
      const opts = (matches as Array<{ id: string; title: string }>)
        .map((m, i) => `${i + 1}. ${m.title}`)
        .join("\n");
  ...
  if (lower === "/recap") {
    if (!workspace_id) {
      await tgSend(
        chat_id,
        "📦 /recap works in workspace-linked groups. Link first with /linkworkspace <code>.",
      );
      return new Response('{"ok":true}', { headers: corsHeaders });
    }
  ...
  if (lower.startsWith("/schedule ")) {
    if (!workspace_id) {
      await tgSend(
        chat_id,
        "🗓 /schedule works in workspace-linked groups. Link the group with /linkworkspace <code> first.",
      );
      return new Response('{"ok":true}', { headers: corsHeaders });
    }
    ...
    const durationMinutes = durMatch
      ? durMatch[2]?.toLowerCase().startsWith("h")
    ...
    for (const m of mentions) {
      const match = (wsMembers || []).find(
        (x: { user_id: string; display_name?: string | null }) =>
          (x.display_name || "").toLowerCase() === m.toLowerCase(),
      );
    ...
    if (missing.length) {
      await tgSend(
        chat_id,
        `⚠️ These folks aren't in the workspace: ${missing.map((x) => `@${x}`).join(", ")}`,
      );
      return new Response('{"ok":true}', { headers: corsHeaders });
    }
    // Include the sender themselves if they're a workspace member.
    if (senderUserId && !participantIds.includes(senderUserId)) {
      const selfIsMember = (wsMembers || []).some(
        (x: { user_id: string }) => x.user_id === senderUserId,
      );
    ...
    if (ranked.length === 0) {
      await tgSend(
        chat_id,
        `😕 No slot works for ${participantIds.length} people in the next week.`,
      );
    } else {
      const lines = ranked.map((s, i) => `${i + 1}. ${s.local}`);
      await tgSend(
        chat_id,
        `<b>🗓 Slots for "${title}" (${durationMinutes}m)</b>\n${lines.join("\n")}\n\nReply with the number to book it.`,
      );
    }
    return new Response('{"ok":true}', { headers: corsHeaders });
  }
  ...
  if (lower === "/undo") {
    ...
    if (!entry) {
      await tgSend(
        chat_id,
        "⏰ Nothing to undo — the 5-minute window has passed or you haven't done anything yet.",
      );
    } else {
  ...
  if (
    ...
    const digest = await buildSharedFamilyDigest(supabase, memberIds, household, {
      limit: 7,
      tz: userTimezone,
      greeting: lower.startsWith("/morning"),
      horizonDays: 14,
    });
  ...
  if (digestVoiceMatch) {
    if (!group) {
      await tgSend(
        chat_id,
        "🎙 Voice digests are a family-group setting. Use /recap in workspace groups.",
      );
      return new Response('{"ok":true}', { headers: corsHeaders });
    }
    await tgSend(
      chat_id,
      await handleToggleGroupSetting(
        supabase,
        group.id,
        "voice_digest_enabled",
        digestVoiceMatch[1] === "on",
        "Voice morning digest",
      ),
    );
    return new Response('{"ok":true}', { headers: corsHeaders });
  }
  ...
  if (
    lower === "/news voice" ||
    /\b(news|headlines)\b.*\b(voice|audio|speak|spoken)\b/i.test(trimmed) ||
    /\b(voice|audio|speak|spoken)\b.*\b(news|headlines)\b/i.test(trimmed)
  ) {
    const { data: prof } = await supabase
      .from("profiles")
  ...
  if (
    /^(show me the calendar|show calendar|what'?s on (my|our) calendar|what do (i|we) have (today|tomorrow|this week)|what are (my|our) next meetings|next meetings|upcoming meetings)[?!.]*$/i.test(
      trimmed,
    )
  ) {
    if (/meeting/i.test(trimmed)) {
      await tgSend(
        chat_id,
        await handleUpcomingMeetings(supabase, memberIds, household, userTimezone),
      );
  ...
  if (lower === "/shopping" || lower === "/list") {
    if (!group) {
      await tgSend(
        chat_id,
        "🛒 Shopping lists are available in family groups. In workspace groups, say “add task …” or use /add.",
      );
      return new Response('{"ok":true}', { headers: corsHeaders });
    }
    const sent = await sendTappableShoppingList(
      chat_id,
      supabase,
      group.owner_user_id,
      TELEGRAM_API_KEY,
  ...
  if (lower === "/linkme") {
    if (!telegram_user_id) {
      ...
      await tgSend(
        chat_id,
        "Link your Telegram account in private chat with Dori, then I can recognize you in this workspace group.",
      );
    } else {
      await supabase.from("telegram_user_map").upsert(
        {
      ...
      await tgSend(
        chat_id,
        `✅ Linked Telegram ID <code>${telegram_user_id}</code> to this family space.`,
      );
    }
    return new Response('{"ok":true}', { headers: corsHeaders });
  }
  ...
  if (lower === "/contracts" || lower === "/expiring") {
    ...
    if (!rows?.length) {
      await tgSend(
        chat_id,
        expiringOnly ? "✅ Nothing expiring in the next 60 days." : "📄 No active contracts.",
      );
      return new Response('{"ok":true}', { headers: corsHeaders });
    }
    await tgSend(
      chat_id,
      `<b>${expiringOnly ? "⏳ Expiring soon (60 days)" : "📄 Active contracts"}</b> — tap for options`,
    );
    for (const c of rows) {
      const cost = c.cost_amount
        ? ` — ${c.cost_amount}€${c.cost_frequency ? "/" + c.cost_frequency : ""}`
  ...
  if (lower === "/snooze") {
    ...
    const { data: rows } = await supabase
      ...
      .eq("user_id", userForChat)
      .eq("completed", false)
      .gte("due_date", t0.toISOString())
      .lt("due_date", t1.toISOString());
    if (!rows?.length) {
      await tgSend(chat_id, "📭 No open tasks today.");
    } else {
      await Promise.all(
        (rows as Array<{ id: string; due_date: string }>).map((r) => {
      ...
      await tgSend(
        chat_id,
        `💤 Snoozed ${rows.length} task${rows.length === 1 ? "" : "s"} to tomorrow.`,
      );
    }
    return new Response('{"ok":true}', { headers: corsHeaders });
  }
  ...
  if (lower === "/load") {
    ...
    if (!data?.length) {
      await tgSend(chat_id, "📭 No meetings this week.");
      return new Response('{"ok":true}', { headers: corsHeaders });
    }
    ...
    Object.entries(totals).forEach(([uid, hrs]) =>
      lines.push(`• ${escapeHtml(household.nameOf(uid))}: ${hrs.toFixed(1)}h`),
    );
    await tgSend(chat_id, lines.join("\n"));
    return new Response('{"ok":true}', { headers: corsHeaders });
  }
  ...
  if (lower.startsWith("/agenda ")) {
    ...
    if (isNaN(date.getTime())) {
      ...
      const [{ data: events }, { data: tasks }] = await Promise.all([
        ...
        supabase
          .from("tasks")
          .select("title, user_id, completed")
          .in("user_id", memberIds)
          .eq("trashed", false)
          .gte("due_date", d0.toISOString())
          .lt("due_date", d1.toISOString()),
      ]);
      const lines = [`<b>📅 ${arg}</b>`];
      if (events?.length) {
        lines.push("\n<b>Events</b>");
        (events as Array<{ title: string; start_time: string; location?: string | null }>).forEach(
          (e) => {
            const t = new Date(e.start_time).toLocaleTimeString("en-GB", {
              hour: "2-digit",
              minute: "2-digit",
            ...
            lines.push(
              `• ${t} — ${escapeHtml(e.title)}${e.location ? ` @ ${escapeHtml(e.location)}` : ""}`,
            );
          },
        );
      }
      if (tasks?.length) {
        lines.push("\n<b>Tasks</b>");
        (tasks as Array<{ title: string; completed: boolean }>).forEach((t) =>
          lines.push(`${t.completed ? "✅" : "⬜"} ${escapeHtml(t.title)}`),
        );
      }
      if (!events?.length && !tasks?.length) lines.push("\n📭 Nothing scheduled.");
      await tgSend(chat_id, lines.join("\n"));
  ...
  if (lower === "/spent" || lower.startsWith("/spent ")) {
    ...
    await tgSend(
      chat_id,
      `💶 Spent <b>€${total.toFixed(2)}</b>${category ? ` on "${escapeHtml(category)}"` : ""} in past ${period} (${data?.length || 0} items).`,
    );
    return new Response('{"ok":true}', { headers: corsHeaders });
  }
  ...
  if (lower === "/weather" || lower.startsWith("/weather ")) {
    ...
    if (!city) {
      await tgSend(chat_id, "Usage: <code>/weather Berlin</code>");
      return new Response('{"ok":true}', { headers: corsHeaders });
    }
    try {
      ...
      if (!place) {
        await tgSend(chat_id, `🌍 Couldn't find "${escapeHtml(city)}".`);
        return new Response('{"ok":true}', { headers: corsHeaders });
      }
      const w = await fetch(
        `https://api.open-meteo.com/v1/forecast?latitude=${place.latitude}&longitude=${place.longitude}&daily=temperature_2m_max,temperature_2m_min,precipitation_sum&timezone=auto&forecast_days=2`,
      ).then((r) => r.json());
  ...
  if (langMatch) {
    ...
    await tgSend(
      chat_id,
      langMatch[1] === "de" ? "🇩🇪 Sprache auf Deutsch gestellt." : "🇬🇧 Language set to English.",
    );
    return new Response('{"ok":true}', { headers: corsHeaders });
  }
    ...
    if (!q) {
      ...
      if (!chores?.length) {
        ...
        if (!rotation.length) {
          await tgSend(
            chat_id,
            `🧹 <b>${escapeHtml(c.title)}</b> — no rotation set; assigned to ${c.family_member_id || "no one"}.`,
          );
        } else {
          // Last completer index → next is +1 mod len
          const { data: last } = await supabase
            .from("family_chore_completions")
            .select("family_member_id")
            .eq("chore_id", c.id)
            .order("completed_at", { ascending: false })
            .limit(1);
          const lastIdx = last?.[0]?.family_member_id
            ? rotation.indexOf(last[0].family_member_id)
            : -1;
          ...
          await tgSend(
            chat_id,
            `🧹 <b>${escapeHtml(c.title)}</b> — next up: <b>${escapeHtml(fm?.name || "Member")}</b>`,
          );
        }
      }
    }
  ...
  if (lower === "/free" || lower.startsWith("/free ")) {
    await tgSend(
      chat_id,
      await handleFreeTime(supabase, userForChat, trimmed.slice(5).trim(), userTimezone),
    );
    return new Response('{"ok":true}', { headers: corsHeaders });
  }
  ...
  if (voiceMatch) {
    if (!group) {
      await tgSend(
        chat_id,
        "🎙 Workspace group voice replies are kept off for now. Voice notes still work as input.",
      );
      return new Response('{"ok":true}', { headers: corsHeaders });
    }
    await tgSend(
  ...
  if (lower.startsWith("/add ")) {
    forcedPrefix = "Add task: ";
    payloadText = trimmed.slice(5);
  } else if (lower.startsWith("/buy ")) {
    forcedPrefix = "Add to shopping: ";
    payloadText = trimmed.slice(5);
  } else if (lower.startsWith("/event ")) {
    forcedPrefix = "Create event: ";
    payloadText = trimmed.slice(7);
  } else if (lower.startsWith("/note ")) {
    forcedPrefix = "Save note: ";
    payloadText = trimmed.slice(6);
  } else if (lower.startsWith("/remind ")) {
    forcedPrefix = "Set reminder: ";
    payloadText = trimmed.slice(8);
  }
  // Phase 4: gap-closure shortcuts → chat function will pick the right tool
  else if (lower === "/budget" || lower.startsWith("/budget ")) {
    forcedPrefix = "Budget: ";
    payloadText = trimmed.slice(7).trim() || "list";
  } else if (lower === "/period" || lower.startsWith("/period ")) {
    forcedPrefix = "Log period: ";
    payloadText = trimmed.slice(7).trim() || "today";
  } else if (lower === "/meds" || lower.startsWith("/meds ")) {
    forcedPrefix = "Log medication: ";
    payloadText = trimmed.slice(5).trim() || "list";
  } else if (lower === "/pantry" || lower.startsWith("/pantry ")) {
    forcedPrefix = "Pantry: ";
    payloadText = trimmed.slice(7).trim() || "list";
  } else if (lower === "/fasting" || lower.startsWith("/fasting ")) {
    forcedPrefix = "Fasting: ";
    payloadText = trimmed.slice(8).trim() || "status";
  } else if (lower.startsWith("/flight ")) {
    forcedPrefix = "Track flight: ";
    payloadText = trimmed.slice(8);
  } else if (lower === "/status" || lower.startsWith("/status ")) {
    forcedPrefix = "Set my presence to: ";
    payloadText = trimmed.slice(7).trim() || "home";
  } else if (lower.startsWith("/zakat ")) {
    forcedPrefix = "Calculate Zakat on: ";
    payloadText = trimmed.slice(7);
  } else if (lower.startsWith("/tz ")) {
    forcedPrefix = "What time is it in: ";
    payloadText = trimmed.slice(4);
  } else if (lower.startsWith("/fx ")) {
    forcedPrefix = "Convert currency: ";
    payloadText = trimmed.slice(4);
  } else if (lower === "/summary" || lower.startsWith("/summary")) {
    forcedPrefix = "Summarise unread emails";
    payloadText = "";
  } else if (lower.startsWith("/subtask ")) {
    forcedPrefix = "Add subtask: ";
    payloadText = trimmed.slice(9);
  } else if (lower.startsWith("/tag ")) {
    forcedPrefix = "Tag task: ";
    payloadText = trimmed.slice(5);
  } else if (lower.startsWith("/estimate ")) {
    forcedPrefix = "Estimate task: ";
    payloadText = trimmed.slice(10);
  }
  ...
  (priorReplies || []).forEach((r: { reply?: string | null; created_at: string }) => {
    if (r.reply)
      turns.push({ role: "assistant", content: r.reply, ts: new Date(r.created_at).getTime() });
  });
