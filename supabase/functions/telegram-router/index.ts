...
// ─── Shared morning digest ──────────────────────────────────────────────
// Returns the next N important upcoming items (events + due tasks) across
// the whole household, ordered chronologically and grouped by day. Used by
// the /digest command and the daily morning cron push.
export async function buildSharedFamilyDigest(
  supabase: any,
  ids: string[],
  household: any,
  opts: { limit?: number; tz?: string; horizonDays?: number; greeting?: boolean } = {},
): Promise<string> {
  ...
  const end = new Date(start); end.setDate(end.getDate() + horizonDays); end.setHours(23, 59, 59, 999);
  ...
  // Score: earlier = better; high-priority tasks get a small bonus.
  items.sort((a, b) => {
