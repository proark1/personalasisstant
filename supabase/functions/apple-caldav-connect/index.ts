  ...
  try {
    ...
    if (propfind.status === 401 || propfind.status === 403) {
      return new Response(
        JSON.stringify({
          error:
            "Invalid Apple ID or app-specific password. Generate a new one at appleid.apple.com.",
        }),
    ...
    if (!propfind.ok && propfind.status !== 207) {
      ...
      return new Response(
        JSON.stringify({ error: `iCloud rejected the connection (${propfind.status}).` }),
        {
          status: 502,
          headers: { ...corsHeaders, "Content-Type": "application/json" },
        },
