  ...
  function: {
    ...
    parameters: {
      ...
      properties: {
        ...
        source_language: { type: 'string', description: 'ISO-639-1 code if foreign-language text was detected.' },
        ...
        receipt: {
          ...
          properties: {
            ...
            category: { type: 'string', description: 'e.g. groceries, restaurant, transport, electronics' },
            line_items: {
              type: 'array',
        ...
        label: {
          ...
          properties: {
            ...
            target_language: { type: 'string', description: 'ISO-639-1 to translate INTO. Default en.' },
          },
        },
        document: {
          type: 'object',
...
serve(async (req) => {
  ...
  try {
    ...
    const sourceLanguage = typeof parsed?.source_language === 'string' ? parsed.source_language.slice(0, 8) : null;
    const confidence = typeof parsed?.confidence === 'number' ? clamp01(parsed.confidence) : null;
    // Pick out the matching per-kind block.
    const extracted = (parsed?.[detectedKind] && typeof parsed[detectedKind] === 'object')
