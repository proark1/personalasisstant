...
serve(async (req) => {
  ...
  try {
    ...
    const [tasksRes, eventsRes, checkinsRes, energyRes, slipRes, statsRes] = await Promise.all([
      ...
      admin.from('dori_slip_risk')
        .select('task_id, slip_risk')
        .eq('user_id', user.id)
        .gt('slip_risk', 0.3)
        .limit(20),
      admin.from('dori_task_stats')
    ...
    const userPrompt = [
      ...
      checkins.map((c) => `- ${c.checkin_date}: energy=${c.energy_level || '?'}, sleep=${c.sleep_hours ?? '?'}h q${c.sleep_quality ?? '?'}, focus="${c.mood ?? ''}"`).join('\n') || '(none)',
      '',
      'Per-category completion stats:',
      stats.map((s) => `- ${s.category}: on-time ${(Number(s.on_time_rate || 0) * 100).toFixed(0)}%, lead ${s.median_lead_hours}h (${s.sample_size} samples)`).join('\n') || '(none)',
    ].filter(Boolean).join('\n');
    ...
    const { data: ins, error: insErr } = await admin.from('schedule_proposals').insert({
      user_id: user.id,
