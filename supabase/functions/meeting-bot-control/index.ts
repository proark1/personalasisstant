...
serve(async (req) => {
  ...
  try {
    ...
    try {
      ...
      return json({ error: "MeetingBot not configured", details: (e as Error).message }, 503);
    }

    if (action === "cancel") {
      if (row.external_bot_id) {
        try {
          await deleteBot(cfg, user.id, row.external_bot_id);
        } catch (e) {
    ...
    try {
      ...
      if (analysis && typeof analysis === "object") {
        ...
        if (typeof analysis.sentiment === "string")
          patch.sentiment = analysis.sentiment.slice(0, 80);
      }
