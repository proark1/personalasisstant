...
Priority: ${task.priority}`;
      ...
      if (overdueTasks.length === 0) {
        return new Response(
          JSON.stringify({ suggestions: [], message: "No overdue tasks to reschedule!" }),
          {
            headers: { ...corsHeaders, "Content-Type": "application/json" },
          },
        );
...
${incompleteTasks
  ...
  .map(
    (t) =>
      `- ID: ${t.id}, Title: "${t.title}", Category: ${t.category}, Priority: ${t.priority}${t.dueDate ? `, Due: ${t.dueDate}` : ""}`,
  )
  .join("\n")}
...
${
  events && events.length > 0
    ? `\nExisting events/commitments:\n${events
        ...
        .map(
          (e) =>
            `- ${e.title}: ${new Date(e.startTime).toLocaleTimeString()} to ${new Date(e.endTime).toLocaleTimeString()}`,
        )
        .join("\n")}`
...
"${dumpContent || ""}"`;
    ...
    if (!response.ok) {
      ...
      // Log failed request
      await logAIUsage(supabaseAdmin, userId, `ai-assistant-${type}`, model, 0, 0, 0, "error", {
        error: errorText,
      });
