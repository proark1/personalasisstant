...
serve(async (req) => {
  ...
  const ownerIds = Array.from(
    new Set((links as WorkspaceLink[]).map((l) => l.workspaces?.owner_id).filter(Boolean)),
  ) as string[];
  ...
  if (ownerIds.length) {
    const { data: profs } = await admin
      ...
      .in("user_id", ownerIds);
    for (const p of (profs || []) as Array<{ user_id: string; timezone?: string | null }>)
      tzByOwner.set(p.user_id, p.timezone || null);
  }
  ...
  if (eligible.length > 0) {
    ...
    alreadySent = new Set(
      (existing || []).map(
        (r: { user_id: string; trigger_key: string }) => `${r.user_id}|${r.trigger_key}`,
      ),
    );
  }
  ...
  for (const r of results) {
    if (r.status === "fulfilled") {
      ...
      else if (r.value.status === "already_sent")
        skipped.push({ workspace_id: r.value.workspace_id, reason: "already_sent" });
      else if (r.value.status === "http_error")
        skipped.push({ workspace_id: r.value.workspace_id, reason: `http_${r.value.http}` });
    } else {
