...
async function getNewMessageIds(accessToken: string, startHistoryId: string): Promise<{ messageIds: string[]; expired: boolean }> {
  try {
  ...
  try {
    ...
    if (storedHistoryId) {
      ...
      if (expired) {
        ...
        return new Response(JSON.stringify({ synced: 0, newEmails: 0, highPriority: 0, syncType: 'incremental' }), {
