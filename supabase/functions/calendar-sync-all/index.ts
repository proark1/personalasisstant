...
serve(async (req) => {
  ...
  for (const conn of (connections || []) as CalendarConnection[]) {
    try {
      ...
      console.error(
        `calendar-sync-all: connection ${conn.id} failed:`,
  ...
  console.log(
    `calendar-sync-all: ${totals.connections} connections, ${totals.imported} imported, ${totals.updated} updated, ${totals.pushed} pushed, ${totals.errors} errors`,
  );
