  ...
  try {
    ...
    try {
      ...
      if (Array.isArray(raw?.templates)) {
        drafts = raw.templates.map((t: { tone?: string; subject?: string; body?: string }) => ({
          tone: t?.tone ?? "formal",
          subject: t?.subject,
          body: t?.body,
        }));
        ...
        for (const k of ["formal", "email", "brief"]) {
          if (raw?.[k])
            drafts.push({
              tone: k,
              subject: raw[k]?.subject,
              body: typeof raw[k] === "string" ? raw[k] : raw[k]?.body,
            });
        }
      }
    } catch (e) {
    ...
    const existingDrafts = Array.isArray(contract.cancellation_drafts)
      ? contract.cancellation_drafts
      : [];
    await admin
      .from("contracts")
      .update({
