    ...
    for (const conn of conns) {
      ...
      while (pages < MAX_PAGES_PER_CONNECTION) {
        try {
          ...
          const code =
            e instanceof Object && "plaidError" in e
              ? (e as { plaidError?: { error_code?: string } }).plaidError?.error_code
              : undefined;
      ...
      if (upserts.length) {
        ...
        const plaidAccountIds = Array.from(
          new Set(
            upserts.map((u) => (u.metadata as { plaid_account_id: string }).plaid_account_id),
          ),
        );
        ...
        const enriched = upserts.map((u) => ({
          ...u,
          account_id:
            accMap.get((u.metadata as { plaid_account_id: string }).plaid_account_id) ?? null,
        }));
      ...
      for (const r of removed) {
        await admin
          ...
          .eq("user_id", user.id)
          .eq("source", "plaid")
          .eq("external_id", r.transaction_id);
      }
