...
serve(async (req) => {
  ...
  try {
    ...
    if (!Number.isFinite(lon) || lon < -180 || lon > 180)
      return json({ error: "invalid lon" }, 400);
      ...
      if (fetchableMissing.length > 0) {
        try {
          const params = new URLSearchParams({
            ...
            daily:
              "temperature_2m_min,temperature_2m_max,precipitation_sum,precipitation_probability_max,wind_speed_10m_max,weather_code",
