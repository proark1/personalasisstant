...
serve(async (req) => {
  ...
  try {
    ...
    const { workspace_id, post_to_telegram = false, timezone } = body as {
      workspace_id?: string; post_to_telegram?: boolean; timezone?: string;
    };
    ...
    if (!tz) {
      ...
      if (ws?.owner_id) {
        const { data: p } = await admin.from('profiles').select('timezone').eq('user_id', ws.owner_id).maybeSingle();
    ...
    if (post_to_telegram) {
      const { data: link } = await admin.from('workspace_telegram_links')
        .select('chat_id')
      ...
      if (link?.chat_id) {
        ...
        } catch (e) { console.error('recap post failed', e); }
      }
    }
