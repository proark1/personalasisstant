...
describe("telegram quick command formatting", () => {
  ...
  it("formats dates in the user timezone and falls back for invalid dates", () => {
    ...
    expect(userDayYmd(new Date("2026-06-10T23:30:00.000Z"), "Definitely/NotAZone")).toBe(
      "2026-06-10",
    );
  });
  ...
  it("scopes memory output and escapes saved facts/preferences", () => {
    const text = buildMemorySnapshotMessage({
      workspaceId: "workspace-1",
      memoryRows: [
        { key: "client <tone>", category: "work & sales", value: "Prefers concise <updates>" },
      ],
      prefRows: [
        {
          key: "meeting_style",
          value: "No calls before 10 & after 17",
          confidence: 0.82,
          times_seen: 3,
        },
      ],
