...
export function findMentionedEntities({
  ...
  return {
    ...
    contracts: contracts.filter(
      (c) => mentionsTerm(lower, c.name) || mentionsTerm(lower, c.provider),
    ),
...
export function buildSmartPayload({
  ...
  const mentionsFamilyMember =
    familyNames.some((name) => lowerMsg.includes(name)) || mentioned.familyMembers.length > 0;

  // Contacts — triggered by location, role keywords, or direct name mention
  const detectedLocations = matchesLocation(lowerMsg);
  const wantsContacts =
    matchesCategory(lowerMsg, CONTEXT_TRIGGERS.contacts) ||
    detectedLocations.length > 0 ||
    mentioned.contacts.length > 0;
  ...
  // Contracts
  if (
    (matchesCategory(lowerMsg, CONTEXT_TRIGGERS.contracts) || mentioned.contracts.length > 0) &&
    contracts &&
    contracts.length > 0
  ) {
  ...
  if (
    (matchesCategory(lowerMsg, CONTEXT_TRIGGERS.notes) || mentioned.notes.length > 0) &&
    notes &&
    notes.length > 0
  ) {
  ...
  if (
    (matchesCategory(lowerMsg, CONTEXT_TRIGGERS.habits) ||
      matchesCategory(lowerMsg, CONTEXT_TRIGGERS.wellbeing)) &&
    habits &&
    habits.length > 0
  ) {
  ...
  // Family context
  if (
    (matchesCategory(lowerMsg, CONTEXT_TRIGGERS.family) ||
      matchesCategory(lowerMsg, CONTEXT_TRIGGERS.shopping) ||
      matchesCategory(lowerMsg, CONTEXT_TRIGGERS.cooking) ||
      mentionsFamilyMember) &&
    familyMembers &&
    ...
    payload.familyContext = {
      ...
      shoppingLists: (shoppingLists || [])
        .filter((l) => !l.is_completed && !l.is_template)
        .map((l) => ({
          name: l.name,
