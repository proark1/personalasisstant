...
const contact = (over: Partial<Contact>): Contact =>
  ({ id: 'c1', name: 'Sarah Chen', company: null, role: null, tags: [], city: null, country: null, email: null, ...over } as unknown as Contact);
const contract = (over: Partial<Contract>): Contract =>
  ({ name: 'Netflix', provider: 'Netflix', category: 'streaming', isActive: true, ...over } as unknown as Contract);
const note = (over: Partial<Note>): Note =>
  ({ id: 'n1', title: 'Apartment lease', content: 'lease ends in March', tags: [], ...over } as unknown as Note);
