...
export function useAssistantConversations() {
  ...
  const [currentConversation, setCurrentConversation] = useState<AssistantConversation | null>(
    null,
  );
  ...
  const startConversation = useCallback(
    async (isStartupBrainstorm = false, title?: string): Promise<string | null> => {
  ...
  const bufferMessage = useCallback((role: "user" | "assistant", content: string) => {
    if (content.trim()) {
      ...
      console.log(
        "[useAssistantConversations] Buffered message:",
        role,
        content.trim().substring(0, 50) + "...",
        "Buffer size:",
        messageBufferRef.current.length,
      );
    }
  }, []);
  ...
  const saveBufferedMessages = useCallback(async (conversationId: string) => {
    console.log(
      "[useAssistantConversations] saveBufferedMessages called with",
      messageBufferRef.current.length,
    ...
    try {
      ...
      const { error } = await supabase.from("assistant_messages").insert(messages);
