...
export function useVisionCapture() {
  ...
  const [phase, setPhase] = useState<{ key: string; label: string; elapsed_ms: number } | null>(
    null,
  );
  ...
  const captureFromFile = useCallback(
    async (file: File, opts?: { hintKind?: VisionKind }): Promise<VisionExtractResult | null> => {
  ...
  const commit = useCallback(
    async (opts?: {
      ...
      try {
        ...
        toast.success(
          created
            ? `Saved as ${humanLabel(created)}`
            : ((dataRecord?.warning as string | undefined) ?? "Saved"),
        );
        reset();
