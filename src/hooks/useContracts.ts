...
export function useContracts(userId: string | undefined) {
  ...
  const mapDbToContract = (row: Tables<"contracts">): Contract => ({
    ...
    reminderSnoozedUntil: row.reminder_snoozed_until
      ? new Date(row.reminder_snoozed_until)
      : undefined,
  ...
  const addContract = useCallback(
    async (input: ContractInput): Promise<Contract | null> => {
      ...
      if (data && !error) {
        ...
        queryClient.setQueryData<Contract[]>(["contracts", userId], (prev) => [
          ...(prev ?? []),
          newContract,
        ]);
  ...
  const updateContract = useCallback(
    async (id: string, updates: Partial<ContractInput>): Promise<boolean> => {
      ...
      if (updates.endDate !== undefined)
        dbUpdates.end_date = updates.endDate?.toISOString().split("T")[0] || null;
      if (updates.renewalDate !== undefined)
        dbUpdates.renewal_date = updates.renewalDate?.toISOString().split("T")[0] || null;
      if (updates.cancellationNoticeDays !== undefined)
        dbUpdates.cancellation_notice_days = updates.cancellationNoticeDays;
      if (updates.autoRenews !== undefined) dbUpdates.auto_renews = updates.autoRenews;
      if (updates.contractNumber !== undefined)
        dbUpdates.contract_number = updates.contractNumber || null;
      ...
      if (!error) {
        ...
        moduleBus.emit(
          "contract:updated",
          { contractId: id, fields: Object.keys(updates) },
          "useContracts",
        );
        return true;
      }
  ...
  const getExpiringContracts = useCallback(
    (withinDays: number = 30) => {
      ...
      return contracts.filter(
        (c) => c.isActive && c.renewalDate && c.renewalDate >= now && c.renewalDate <= threshold,
      );
    },
    [contracts],
  );
  ...
  const getCancellationDeadlines = useCallback(
    (withinDays: number = 30) => {
      const now = new Date();
      const threshold = new Date();
      threshold.setDate(threshold.getDate() + withinDays);
