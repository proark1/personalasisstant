...
export function useAppNotifications() {
  ...
  const createNotification = useCallback(
    async (options: CreateNotificationOptions) => {
  ...
  const scheduleNativeNotification = useCallback(
    async (options: ScheduleNativeNotificationOptions): Promise<number | null> => {
      if (!isNative) {
  ...
  const notifyTaskDueSoon = useCallback(
    (taskTitle: string, taskId: string, dueIn: string) => {
      return createNotification({
        type: "reminder",
        title: "Task Due Soon",
  ...
  const notifyEventReminder = useCallback(
    (eventTitle: string, eventId: string, startsIn: string) => {
      return createNotification({
        type: "reminder",
  ...
  const notifyAppointmentCreated = useCallback(
    async (
      ...
      if (notificationTime > new Date()) {
        const notificationId =
          parseInt(appointmentId.replace(/-/g, "").slice(0, 8), 16) % 2147483647;
  ...
  const notifyAppointmentUpdated = useCallback(
    (appointmentTitle: string, appointmentId: string) => {
      return createNotification({
        type: "event",
  ...
  const notifyContractCreated = useCallback(
    (contractName: string, contractId: string) => {
      return createNotification({
        type: "contract",
  ...
  const notifyContractRenewal = useCallback(
    (contractName: string, contractId: string, daysUntil: number) => {
      return createNotification({
        type: "contract",
  ...
  const notifyContactReminder = useCallback(
    (contactName: string, contactId: string, reason?: string) => {
      return createNotification({
        type: "contact",
  ...
  const notifyHabitCompleted = useCallback(
    (habitName: string, habitId: string, streak: number) => {
      return createNotification({
        type: "info",
  ...
  const notifyItemShared = useCallback(
    (itemType: string, itemName: string, sharedBy: string) => {
      return createNotification({
        type: "share",
