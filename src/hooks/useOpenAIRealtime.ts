...
export function useOpenAIRealtime({
  ...
  // Latest-ops ref. handleFunctionCall used to depend on all 33 of these
  // (contextData + every CRUD/refetch callback), so it was recreated on
  // every render — which churned `connect`'s identity and destabilized
  // the WebRTC connection. We stash the live values in a ref updated each
  ...
  const handleFunctionCall = useCallback(async (name: string, args: any, callId: string) => {
    ...
    // therefore connect) referentially stable across re-renders.
  }, []);
