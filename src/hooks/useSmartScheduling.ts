...
export function useSmartScheduling() {
  ...
  const analyzePatterns = useCallback(async () => {
    ...
    try {
      ...
      const avgTasksPerPeakHour =
        peakHours.length > 0 ? Math.round(totalPeakHourTasks / peakHours.length) : 0;
  ...
  const getSuggestionsForTask = useCallback(
    (taskPriority: string, _estimatedDuration?: number): SchedulingSuggestion[] => {
      if (!patterns) return [];
      ...
      if (taskPriority === "high" && patterns.peakHours.length > 0) {
        const nextPeakHour =
          patterns.peakHours.find((h) => h > currentHour) || patterns.peakHours[0];
