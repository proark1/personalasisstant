...
export function useGroupChat(userId: string | null) {
  ...
  const decryptMessage = useCallback(
    async (msg: GroupMessage): Promise<GroupMessage> => {
  ...
  const createGroup = useCallback(
    async (name: string, memberIds: string[], description?: string) => {
      ...
      try {
        ...
        const { error: adminError } = await supabase.from("chat_group_members").insert({
          group_id: group.id,
          user_id: userId,
          role: "admin",
        });
  ...
  const sendMessage = useCallback(
    async (groupId: string, content: string, attachments: ChatAttachment[] = []) => {
  ...
  const addReaction = useCallback(
    async (messageId: string, emoji: string, isGroup: boolean) => {
      if (!userId) return;
