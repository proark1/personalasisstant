...
export function useCallRecording(
  ...
  ); // intentionally excludes uploadRecording — defined after this callback to avoid circular deps
  ...
  const uploadRecording = useCallback(
    async (blob: Blob, duration: number) => {
      ...
      try {
        ...
        const { error } = await supabase.storage.from("call-recordings").upload(fileName, blob, {
