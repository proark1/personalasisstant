...
export function useEmails({ enabled = true, autoSync = true }: UseEmailsOptions = {}) {
  ...
  const syncEmails = useCallback(async () => {
    ...
    try {
      ...
      if (data?.error === "gmail_scope_missing") {
        toast.error("Gmail access not authorized.");
        return;
      }
      if (data?.error) {
  ...
  const batchUpdateEmails = useCallback(
  ...
  const markImportant = useCallback(
    async (emailId: string) => {
      ...
      await updateEmail(emailId, {
        ...
        priority_score: email.is_important ? 3 : 1,
      });
    },
    [emails, updateEmail],
  );
  ...
  const snoozeEmail = useCallback(
    async (emailId: string, until: Date) => {
  ...
  const categorizeEmail = useCallback(
    async (emailId: string, priority: "high" | "medium" | "low" | "spam") => {
      if (!user) return;
      const email = emails.find((e) => e.id === emailId);
      if (!email) return;
      ...
      if (domain) {
        try {
          const rulePayload = {
            ...
            default_priority: updates.priority_score,
          } as any;
          await supabase
            .from("email_sender_rules")
            .upsert(rulePayload, { onConflict: "user_id,sender_pattern" });
        } catch (e) {
  ...
  const createSenderRule = useCallback(
    async (
      senderPattern: string,
      rule: { default_category?: string; default_priority?: number; auto_archive?: boolean },
    ) => {
      if (!user) return;
      try {
        const { error } = await supabase
  ...
  const fetchEmailBody = useCallback(async (gmailMessageId: string): Promise<string | null> => {
    try {
      const { data, error } = await supabase.functions.invoke("gmail-fetch-email", {
        body: { messageId: gmailMessageId },
      });
      if (error) throw error;
  ...
  const activeEmails = useMemo(() => {
    ...
    return emails.filter((e) => {
      if (e.user_snoozed_until) return new Date(e.user_snoozed_until) <= now;
      return true;
    });
  }, [emails]);
  ...
  const grouped = useMemo(() => {
    ...
    const attention = clean
      .filter(
        (e) =>
          e.category === "action_required" ||
          e.priority_score <= 2 ||
          e.is_important ||
          e.sentiment === "urgent",
      )
      .sort((a, b) => a.priority_score - b.priority_score);
    const fyi = clean.filter(
      (e) => !attention.includes(e) && !["newsletter", "promotion"].includes(e.category),
    );
  ...
  const viewEmails = useMemo(() => {
    ...
    if (view === "flagged")
      return groupByThread(searched.filter((e) => e.is_spam || e.is_phishing));
  ...
  const priorityCount = activeEmails.filter(
    (e) => (e.priority_score <= 2 || e.is_important) && !e.is_read,
  ).length;
