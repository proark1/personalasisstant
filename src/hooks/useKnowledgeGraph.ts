...
export function useKnowledgeGraph(opts: UseKnowledgeGraphOptions = {}) {
  ...
  const fetchNeighbors = useCallback(
    async (entityId: string, limit = 12): Promise<KgNeighbor[]> => {
      if (!user?.id) return [];
      try {
  ...
  const fetchMentions = useCallback(
    async (entityId: string, limit = 30): Promise<KgMention[]> => {
      if (!user?.id) return [];
      try {
        type KgMentionRow = {
          id: string;
                ...
                eq: (
                  ...
                  eq: (
                    ...
                    order: (
                      ...
                      limit: (
                        n: number,
                      ) => Promise<{ data: KgMentionRow[] | null; error: unknown }>;
                    };
                  };
                };
              };
  ...
  const forgetEntityDeep = useCallback(
  ...
  const forgetEntityShallow = useCallback(
    async (entityId: string, reason?: string) => {
      if (!user?.id) return null;
      try {
        const { data, error: invErr } = await supabase.functions.invoke("memory-forget", {
