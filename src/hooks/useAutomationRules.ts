...
export type ActionType = "add_xp" | "create_reminder" | "show_notification" | "log_activity";
...
export function useAutomationRules() {
  ...
  const updateRule = useCallback(
    async (ruleId: string, updates: Partial<CreateRuleInput & { isActive: boolean }>) => {
      if (!user) return;
      ...
      try {
        ...
        if (updates.conditionConfig !== undefined)
          updateData.condition_config = updates.conditionConfig;
  ...
  const checkTrigger = useCallback(
    (triggerType: TriggerType, _context?: Record<string, unknown>) => {
      const matchingRules = rules.filter(
        (rule) => rule.isActive && rule.triggerType === triggerType,
      );
