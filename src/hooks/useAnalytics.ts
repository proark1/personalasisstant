...
export function useAnalytics() {
  ...
  const trackEvent = useCallback(
    async ({ category, action, label, value, metadata = {} }: TrackEventOptions) => {
      if (!user) return;
  ...
  const trackCreate = useCallback(
    (category: EventCategory, itemId: string, metadata?: Record<string, unknown>) => {
  ...
  const trackAIUsage = useCallback(
    (functionName: string, metadata?: Record<string, unknown>) => {
