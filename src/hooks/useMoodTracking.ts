...
export function useMoodTracking() {
    ...
    try {
      ...
      if (data) {
        ...
        if (mappedLogs.length > 0) {
          ...
          const avgEnergy =
            mappedLogs.reduce((sum, l) => sum + l.energyScore, 0) / mappedLogs.length;
          ...
          setStats({
            ...
            moodTrend:
              recentMoodAvg > olderMoodAvg + 0.3
                ? "improving"
                : recentMoodAvg < olderMoodAvg - 0.3
                  ? "declining"
                  : "stable",
            energyTrend:
              recentEnergyAvg > olderEnergyAvg + 0.3
                ? "improving"
                : recentEnergyAvg < olderEnergyAvg - 0.3
                  ? "declining"
                  : "stable",
  ...
  const logMood = useCallback(
    async (moodScore: number, energyScore: number, contextTags: string[] = [], notes?: string) => {
      if (!user) return;
