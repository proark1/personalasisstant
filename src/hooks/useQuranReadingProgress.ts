...
export function useQuranReadingProgress() {
  ...
  const fetchProgress = useCallback(async () => {
    ...
    try {
      ...
      for (let i = 6; i >= 0; i--) {
        ...
        const dayData = allData?.filter(p => 
          p.read_at >= dayStart && p.read_at <= dayEnd
        ) || [];
        weekStats.push({
          date: format(day, 'EEE'),
  ...
  const markAyahAsRead = useCallback(async (surahNumber: number, ayahNumber: number) => {
    if (!user) return;
  ...
  const markSurahAsRead = useCallback(async (surahNumber: number, totalAyahs: number) => {
    if (!user) return;
  ...
  const updateGoal = useCallback(async (updates: Partial<Omit<ReadingGoal, 'id'>>) => {
    if (!user || !goal) return;
  ...
  const isAyahRead = useCallback((surahNumber: number, ayahNumber: number) => {
    return allProgress.some(p => p.surah_number === surahNumber && p.ayah_number === ayahNumber);
  }, [allProgress]);

  const getSurahProgress = useCallback((surahNumber: number, totalAyahs: number) => {
