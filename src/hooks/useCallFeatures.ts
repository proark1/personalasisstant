...
export function useCallFeatures() {
  ...
  const fetchScheduledCalls = useCallback(async () => {
    ...
    if (!error && data) {
      ...
      const allParticipantIds = [...new Set(data.flatMap(c => [c.organizer_id, ...c.participant_ids]))];
      const { data: profiles } = await supabase
        .from('profiles')
        .select('user_id, display_name')
  ...
  // Fetch call notes
  const fetchCallNotes = useCallback(async (sessionId: string) => {
    if (!user) return;
  ...
  // Add call note
  const addCallNote = useCallback(async (sessionId: string, content: string) => {
    if (!user) return null;
  ...
  // Update call note
  const updateCallNote = useCallback(async (noteId: string, content: string, sessionId: string) => {
    const { error } = await supabase
      .from('call_notes')
  ...
  // Delete call note
  const deleteCallNote = useCallback(async (noteId: string, sessionId: string) => {
    const { error } = await supabase
      .from('call_notes')
      .delete()
      .eq('id', noteId);
