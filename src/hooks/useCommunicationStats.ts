...
async function fetchRecentActivity(userId: string): Promise<RecentActivity[]> {
  ...
  for (const m of msgRes.data || [])
    msgByDay.set(dayKey(m.created_at), (msgByDay.get(dayKey(m.created_at)) || 0) + 1);
  for (const c of callRes.data || [])
    callByDay.set(dayKey(c.created_at), (callByDay.get(dayKey(c.created_at)) || 0) + 1);
...
export function useCommunicationStats() {
  ...
  const calculateDashboard = useCallback(
    (statsData: CommunicationStat[], recentActivity: RecentActivity[]) => {
      const totalMessages = statsData.reduce(
        (sum, s) => sum + s.totalMessagesSent + s.totalMessagesReceived,
        0,
      );
      const totalCalls = statsData.reduce((sum, s) => sum + s.totalCalls, 0);
      const totalCallMinutes =
        statsData.reduce((sum, s) => sum + s.totalCallDurationSeconds, 0) / 60;

      const avgTimes = statsData
        .filter((s) => s.avgResponseTimeSeconds)
        .map((s) => s.avgResponseTimeSeconds!);
    ...
    async (
      ...
      const newStats = {
        ...
        total_messages_received:
          (existing?.total_messages_received || 0) + (update.messagesReceived || 0),
        total_calls: (existing?.total_calls || 0) + (update.calls || 0),
        total_call_duration_seconds:
          (existing?.total_call_duration_seconds || 0) + (update.callDurationSeconds || 0),
  ...
  const getResponseTimeBreakdown = useCallback(() => {
    const fast = stats.filter(
      (s) => s.avgResponseTimeSeconds && s.avgResponseTimeSeconds < 300,
    ).length;
    const medium = stats.filter(
      (s) =>
        s.avgResponseTimeSeconds &&
        s.avgResponseTimeSeconds >= 300 &&
        s.avgResponseTimeSeconds < 3600,
    ).length;
    const slow = stats.filter(
      (s) => s.avgResponseTimeSeconds && s.avgResponseTimeSeconds >= 3600,
    ).length;
