...
export function useAdminDataExport() {
  ...
  const appendExportLog = (line: string) => setExportLogs((prev) => [...prev.slice(-99), line]);
  const appendImportLog = (line: string) => setImportLogs((prev) => [...prev.slice(-199), line]);

  const runExport = useCallback(async (opts: { includeAuthUsers: boolean }) => {
    setExporting(true);
    setExportLogs([]);
  ...
  const runImport = useCallback(async (bundle: ExportBundle, opts: { mode: ImportMode }) => {
    ...
    try {
      ...
      for (const tableName of tableNames) {
        ...
        const writeMode =
          opts.mode === "insert" ? "insert" : opts.mode === "replace" ? "insert" : "upsert";
        ...
        totalErrors += errs;
        appendImportLog(
          `✓ ${tableName}: ${inserted}/${rows.length} rows${errs ? ` (${errs} errors)` : ""}`,
        );
        setImportProgress((p) => ({ ...p, tablesDone: p.tablesDone + 1 }));
      }
