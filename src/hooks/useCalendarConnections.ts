  ...
  const fetchConnections = useCallback(async () => {
    ...
    try {
      const { data, error } = await supabase
        ...
        .select(
          "id, user_id, provider, auth_type, name, color, calendar_id, external_calendar_id, sync_enabled, sync_direction, last_synced_at, last_sync_error, is_default, created_at",
        )
        .eq("user_id", user.id)
    ...
    try {
      ...
      if (data?.code === "NOT_CONFIGURED") {
        toast({
          ...
          description:
            "Microsoft sign-in is being configured. Try again shortly or use ICS import for now.",
          variant: "destructive",
        });
        return;
      }
  ...
  const connectApple = async (appleId: string, appPassword: string, calendarName?: string) => {
    ...
    try {
      ...
      toast({
        ...
        description: await describeEdgeError(
          error,
  ...
  const disconnectCalendar = async (connectionId: string) => {
    try {
      ...
      toast({
        ...
        description:
          (error instanceof Error ? error.message : null) || "Failed to disconnect calendar.",
        variant: "destructive",
      });
    }
  };
  ...
  const toggleSync = async (connectionId: string, enabled: boolean) => {
    try {
      ...
      toast({
        title: "Update failed",
        description:
          (error instanceof Error ? error.message : null) || "Failed to update sync setting.",
        variant: "destructive",
      });
    }
  };
