...
export function useDatabase(userId: string | undefined) {
  ...
  const tasksQuery = useQuery({
    ...
    queryFn: async (): Promise<Task[]> => {
      ...
      if (error) {
        moduleHealth.reportError("tasks", error);
        throw error;
      }
  ...
  const eventsQuery = useQuery({
    ...
    queryFn: async (): Promise<CalendarEvent[]> => {
      ...
      if (error) {
        moduleHealth.reportError("events", error);
        throw error;
      }
  ...
  const flushRunner = useCallback(async (entry: offlineQueue.OfflineQueueEntry) => {
    ...
    if (entry.op === "insert") {
      ...
      const { error } = (await supabase.from(table).insert([entry.payload as any])) as {
        error: Error | null;
      };
  ...
  const addTask = useCallback(
    async (task: Omit<Task, "id" | "createdAt">): Promise<Task> => {
      ...
      if (isOffline()) {
        ...
        moduleBus.emit(
          "task:created",
          { taskId: optimisticTask.id, projectId: optimisticTask.projectId },
          "useDatabase",
        );
  ...
  const updateTask = useCallback(
    async (id: string, updates: Partial<Task>) => {
      ...
      if (updates.recurrenceEnd !== undefined)
        dbUpdates.recurrence_end = updates.recurrenceEnd?.toISOString();
      ...
      if (updates.mainResponsibleId !== undefined)
        dbUpdates.main_responsible_id = updates.mainResponsibleId;
      if (updates.secondaryResponsibleId !== undefined)
        dbUpdates.secondary_responsible_id = updates.secondaryResponsibleId;
      ...
      if (isOffline()) {
        ...
        if (updates.completed === true) {
          ...
          moduleBus.emit(
            "task:updated",
            { taskId: id, fields: Object.keys(updates) },
            "useDatabase",
          );
        }
        return;
  ...
  const emptyTrash = useCallback(async () => {
    ...
    const { error } = await supabase.from("tasks").delete().in("id", trashedIds);
  ...
  const reorderTasks = useCallback(
    async (taskOrders: { id: string; sortOrder: number }[]) => {
  ...
  const addEvent = useCallback(
    async (event: Omit<CalendarEvent, "id">): Promise<CalendarEvent | null> => {
      ...
      if (isOffline()) {
        ...
        setEventsCache((prev) =>
          [...prev, optimisticEvent].sort((a, b) => a.startTime.getTime() - b.startTime.getTime()),
        );
      ...
      if (data && !error) {
        ...
        setEventsCache((prev) =>
          [...prev, newEvent].sort((a, b) => a.startTime.getTime() - b.startTime.getTime()),
        );
  ...
  const updateEvent = useCallback(
    async (id: string, updates: Partial<CalendarEvent>) => {
      ...
      if (isOffline()) {
        ...
        moduleBus.emit(
          "event:updated",
          { eventId: id, fields: Object.keys(updates) },
          "useDatabase",
        );
        return;
      }
  ...
  const shareItem = useCallback(
    async (
      ...
      if (!profiles) {
        return {
          error: `No account found with email "${shareWithEmail}". Make sure the user has registered and the email is correct.`,
        };
      }
  ...
  const getSharedWith = useCallback(
    async (itemType: "task" | "event" | "contract" | "contact", itemId: string) => {
