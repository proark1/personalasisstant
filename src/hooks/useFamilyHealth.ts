...
export function useFamilyHealth() {
  ...
  const fetchAll = useCallback(async () => {
    ...
    try {
      const [docs, emer, ins] = await Promise.all([
        supabase.from('family_important_documents').select('*').eq('user_id', user.id).order('expiry_date', { ascending: true, nullsFirst: false }),
        supabase.from('family_emergency_contacts').select('*').eq('user_id', user.id).order('priority'),
  ...
  const addDocument = async (doc: Omit<ImportantDocument, 'id' | 'user_id'>) => {
    if (!user) return null;
    const { data, error } = await supabase.from('family_important_documents')
      .insert({ ...doc, user_id: user.id }).select().single();
    if (error) { toast.error('Failed to add document'); return null; }
  ...
  const addEmergencyContact = async (c: Omit<EmergencyContact, 'id' | 'user_id'>) => {
    if (!user) return null;
    const { data, error } = await supabase.from('family_emergency_contacts')
      .insert({ ...c, user_id: user.id }).select().single();
    if (error) { toast.error('Failed to add contact'); return null; }
    setEmergencyContacts(prev => [...prev, data as EmergencyContact].sort((a, b) => a.priority - b.priority));
  ...
  const addInsurance = async (i: Omit<InsurancePolicy, 'id' | 'user_id'>) => {
    if (!user) return null;
    const { data, error } = await supabase.from('family_insurance')
      .insert({ ...i, user_id: user.id }).select().single();
    if (error) { toast.error('Failed to add insurance'); return null; }
  ...
  return {
    documents, emergencyContacts, insurance, expiringDocuments,
    isLoading, addDocument, addEmergencyContact, addInsurance, refetch: fetchAll,
  };
}
