...
export function useMessageFeatures() {
  ...
  const pinMessage = useCallback(
    async (messageId: string, messageType: "direct" | "group", chatId: string) => {
      if (!user) return;
  ...
  const starMessage = useCallback(
    async (messageId: string, messageType: "direct" | "group") => {
      if (!user) return;
  ...
  const editMessage = useCallback(
    async (messageId: string, newContent: string, messageType: "direct" | "group") => {
      ...
      const { error } =
        messageType === "direct"
          ? await supabase
              ...
              .update(patch as TablesUpdate<"direct_messages">)
              .eq("id", messageId)
          : await supabase
              .from("group_messages")
              .update(patch as TablesUpdate<"group_messages">)
              .eq("id", messageId);
  ...
  const searchMessages = useCallback(
    async (query: string, _chatPartnerId?: string) => {
