...
export function useFamilyContext({
  ...
  return useMemo(() => {
    ...
    if (children.length > 0) {
      summaryParts.push(
        `Children: ${children.map((c) => `${c.name} (${c.age || "?"} years old${c.school ? `, ${c.grade} at ${c.school}` : ""}${c.kindergarten ? `, at ${c.kindergarten}` : ""})`).join(", ")}`,
      );
    }
    ...
    if (todayEvents.length > 0) {
      summaryParts.push(
        `Today's family schedule: ${todayEvents
          .map((e) => {
    ...
    const allActivities = children.flatMap((c) =>
      c.activities.map(
        (a) => `${c.name}'s ${a.name} (${a.schedule}${a.location ? ` at ${a.location}` : ""})`,
      ),
    );
...
export function buildFamilyContextForAI(context: FamilyContext): string {
  ...
  if (context.schedule.todayEvents.length > 0) {
    ...
    for (const event of context.schedule.todayEvents) {
      ...
      parts.push(
        `- ${event.title}: ${time} - ${endTime}${event.location ? ` at ${event.location}` : ""}`,
      );
    }
  }
  ...
  if (context.schedule.upcomingBirthdays.length > 0) {
    ...
    for (const bday of context.schedule.upcomingBirthdays) {
      ...
      parts.push(
        `- ${bday.member} turns ${bday.age} in ${daysUntil} days (${bday.date.toLocaleDateString()})`,
      );
    }
  }
  ...
  if (context.shopping.activeLists.length > 0) {
    ...
    parts.push(
      context.shopping.activeLists
        .map((l) => `- ${l.name}${l.due_date ? ` (due ${l.due_date})` : ""}`)
        .join("\n"),
    );
  }
