...
export interface Milestone { id: string; user_id: string; title: string; description: string | null; occurred_on: string; category: string | null; related_people: unknown; photo_url: string | null; }
...
export function useJournal() {
  ...
  const refresh = async () => {
    ...
    try {
      ...
      setEntries((e.data as JournalEntry[]) || []); setMilestones((m.data as Milestone[]) || []); setBucket((b.data as BucketItem[]) || []);
  ...
  const addMilestone = async (p: Partial<Milestone>) => { if (!user) return; const { error } = await supabase.from('life_milestones').insert({ ...p as any, user_id: user.id, title: p.title!, occurred_on: p.occurred_on! }); if (error) return toast.error(error.message); toast.success('Added'); refresh(); };
