...
export function useContextualActions(onNavigate?: (panel: string) => void) {
    ...
    try {
      ...
      const [
        ...
        supabase
          ...
          .select("id", { count: "exact", head: true })
          .eq("user_id", user.id)
          .lt("last_contacted_at", format(addDays(new Date(), -30), "yyyy-MM-dd")),
        supabase
          .from("events")
      ...
      if (eventsResult.data && eventsResult.data.length > 0) {
        const contactsForMatch = await supabase
          .from("user_contacts")
          .select("name")
          .eq("user_id", user.id)
          .limit(50);
        if (contactsForMatch.data) {
          for (const event of eventsResult.data) {
            const match = contactsForMatch.data.find((c: { name?: string }) =>
              event.title?.toLowerCase().includes(c.name?.toLowerCase()),
            );
            if (match) {
              meetingContactName = match.name;
              break;
            }
          }
        }
      ...
      const urgentContractDays = urgentContract?.renewal_date
        ? Math.max(
            0,
            Math.floor(
              (new Date(urgentContract.renewal_date).getTime() - new Date().getTime()) /
                (1000 * 60 * 60 * 24),
            ),
          )
        : 0;
  ...
  const getTopActions = useCallback(
    (count: number = 5) => {
