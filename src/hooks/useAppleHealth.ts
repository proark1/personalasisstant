...
const logHealthDebug = (label: string, extra?: Record<string, unknown>) => {
  try {
    ...
    if (isIOS && isNative && isRemoteWebview) {
      toast.error(
        "This iPhone build is loading a remote site (hot-reload). For HealthKit, rebuild with bundled app assets.",
      );
    }
  } catch {
    // ignore
...
export function useAppleHealth() {
  ...
  const fetchHealthMetrics = useCallback(
    async (dateRangeStart?: string, dateRangeEnd?: string) => {
      if (!user?.id) return;
      ...
      try {
        ...
        const todayMetrics = (data || []).filter((m) => m.recorded_at.startsWith(today));
  ...
  const checkHealthKitSetup = useCallback(async (): Promise<{
    available: boolean;
    reason?: string;
  }> => {
    if (!isIOS) {
    ...
    if (!loaded || !Health) {
      return {
        available: false,
        reason: "HealthKit plugin not loaded. Rebuild app with native code.",
      };
    }
    ...
    try {
      ...
      if (!result?.available) {
        return {
          available: false,
          reason:
            "HealthKit not available. Make sure HealthKit capability is enabled in Xcode (Target → Signing & Capabilities → + Capability → HealthKit).",
        };
      }
      ...
      if (
        ...
        return {
          available: false,
          reason:
            "HealthKit not configured in Xcode. Open your iOS project in Xcode, add HealthKit capability, and rebuild.",
        };
      }
  ...
  const runHealthKitDiagnostics = useCallback(async (): Promise<HealthDebugInfo> => {
    ...
    if (!loaded || !Health) {
      newDebugInfo.lastError =
        "HealthKit plugin failed to load. Native module not bundled in this build.";
      setDebugInfo(newDebugInfo);
      return newDebugInfo;
    }
    ...
    try {
      ...
      if (!availResult?.available) {
        newDebugInfo.lastError =
          "HealthKit not available. Check Xcode: Target → Signing & Capabilities → HealthKit enabled.";
        setDebugInfo(newDebugInfo);
        return newDebugInfo;
      }
    } catch (e) {
    ...
    try {
      ...
      if (stepsResult?.value && stepsResult.value > 0) {
        ...
        newDebugInfo.lastError =
          "Query returned no data. Check Health app → Sharing → Apps → DarAI has categories enabled.";
      }
    } catch (e) {
      const errMsg = e instanceof Error ? e.message : String(e);
  ...
  const requestAppleHealthPermission = useCallback(async () => {
    ...
    try {
      ...
      if (errorMsg.includes("denied") || errorMsg.includes("not authorized")) {
        toast.error(
          "Health access denied. Check permissions in: Health app → Sharing → Apps → DarAI",
        );
      } else if (errorMsg.includes("not implemented") || errorMsg.includes("undefined")) {
        toast.error(
          "HealthKit not configured. Rebuild in Xcode with HealthKit capability enabled.",
        );
      } else {
  ...
  const syncAppleHealthInternal = async (daysBack: number = 365) => {
    ...
    logHealthDebug("sync_internal_start", {
      daysBack,
    ...
    for (const { type, metricType, unit } of latestSampleTypes) {
      try {
        ...
        logHealthDebug(`latest_${type}_error`, {
    ...
    try {
      ...
      logHealthDebug("steps_response", {
        count: stepsData?.aggregatedData?.length || 0,
        raw: stepsData,
      });
    ...
    try {
      ...
      logHealthDebug("heart_rate_response", {
        count: heartRateData?.aggregatedData?.length || 0,
        raw: heartRateData,
      });
    ...
    try {
      ...
      logHealthDebug("sleep_response", {
        count: sleepData?.aggregatedData?.length || 0,
        raw: sleepData,
      });
        ...
        for (const bucket of sleepData.aggregatedData) {
          if (bucket.value && bucket.value > 0) {
            ...
            if (bucket.value > 1440) {
              ...
              hours = bucket.value;
              logHealthDebug("sleep_conversion", {
                raw: bucket.value,
                assumed: "hours_decimal",
                hours,
              });
            } else {
    ...
    try {
      ...
      logHealthDebug("respiratory_rate_error", {
        error: e instanceof Error ? e.message : String(e),
      });
    }
  ...
  const addManualMetric = useCallback(
    async (metricType: string, value: number, unit: string, recordedAt?: Date) => {
      if (!user?.id) return null;
  ...
  const openAppSettings = useCallback(async () => {
    ...
    try {
      ...
      toast.error(
        "Could not open settings. Go to Settings → Apps → Health → Access and Devices → DarAI manually.",
      );
    }
  }, []);
