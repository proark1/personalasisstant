...
import { Download, Upload, Database, AlertTriangle, CheckCircle2, XCircle, FileJson } from 'lucide-react';
      ...
      <div className="flex items-start gap-3">
        ...
        <div>
          ...
          <p className="text-sm text-muted-foreground">
            ...
            (and optionally auth users), or restore from a previous snapshot. Admin only — runs
          ...
          <CardContent className="space-y-4">
            <div className="flex items-center justify-between rounded-md border border-border/50 p-3">
              <div className="space-y-0.5">
                <Label htmlFor="include-auth" className="text-sm">Include auth.users</Label>
          ...
          <CardContent className="space-y-4">
            <div className="rounded-md border border-border/50 p-3 space-y-3">
              <div className="space-y-1.5">
                ...
                <p className="text-xs text-muted-foreground">
                  {importMode === 'upsert' && 'Merge by primary key — existing rows updated, new ones inserted.'}
              ...
              <div className="rounded-md border border-border/50 p-3 text-sm space-y-1">
                ...
                {importSummary.totalErrors > 0 && (
                  <div className="flex items-center gap-2 text-amber-500">
                    ...
                    {importSummary.totalErrors} row error{importSummary.totalErrors === 1 ? '' : 's'} — see log
                  </div>
                )}
                {importSummary.tableErrors > 0 && (
                  <div className="flex items-center gap-2 text-destructive">
                    <XCircle className="h-4 w-4" />
                    {importSummary.tableErrors} table{importSummary.tableErrors === 1 ? '' : 's'} skipped
                  </div>
                )}
                <p className="text-xs text-muted-foreground">
        ...
        <CardContent className="p-4 flex gap-3">
          ...
          <div className="text-sm space-y-2">
            ...
            <ul className="space-y-1 text-muted-foreground text-xs list-disc list-inside">
              <li>
                <Badge variant="outline" className="mr-1">Upsert</Badge>
                safest default; existing rows update in place by primary key.
              </li>
              <li>
                <Badge variant="outline" className="mr-1">Replace</Badge>
                wipes each target table before writing. Use only on an empty / staging database.
              </li>
              <li>
                <Badge variant="outline" className="mr-1">Insert</Badge>
