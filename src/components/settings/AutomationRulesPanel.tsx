...
const TRIGGERS: { value: TriggerType; label: string; description: string }[] = [
  ...
  { value: 'focus_session_ended', label: 'Focus Session Ended', description: 'When a focus session completes' },
  { value: 'prayer_logged', label: 'Prayer Logged', description: 'When you log a prayer' },
  { value: 'checkin_completed', label: 'Check-in Completed', description: 'When you complete a daily check-in' },
  { value: 'goal_progress', label: 'Goal Progress', description: 'When you make progress on a goal' },
];
...
const ACTIONS: { value: ActionType; label: string; description: string }[] = [
  ...
  { value: 'show_notification', label: 'Show Notification', description: 'Display a notification message' },
  { value: 'create_reminder', label: 'Create Reminder', description: 'Create a follow-up reminder' },
...
export function AutomationRulesPanel() {
  ...
  const handleCreate = async () => {
    ...
    setNewRule({ name: '', triggerType: '' as TriggerType, actionType: '' as ActionType, actionConfig: {} });
    setIsCreating(false);
  };
              ...
              <div className="space-y-4 py-4">
                ...
                <div className="space-y-2">
                  ...
                  <Select 
                    ...
                    onValueChange={(v) => setNewRule(prev => ({ ...prev, triggerType: v as TriggerType }))}
                  >
                    <SelectTrigger>
                ...
                <div className="space-y-2">
                  ...
                  <Select 
                    ...
                    onValueChange={(v) => setNewRule(prev => ({ ...prev, actionType: v as ActionType }))}
                  >
                    <SelectTrigger>
                  ...
                  <div className="space-y-2">
                    ...
                    <Input 
                      ...
                      onChange={(e) => setNewRule(prev => ({ 
                        ...prev, 
                        actionConfig: { ...prev.actionConfig, amount: parseInt(e.target.value) || 10 }
                      }))}
                    />
                  </div>
                )}
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-xs text-muted-foreground mt-0.5">
                      When <span className="font-medium">{trigger?.label.toLowerCase()}</span> → <span className="font-medium">{action?.label.toLowerCase()}</span>
                    </p>
