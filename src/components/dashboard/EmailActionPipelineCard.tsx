...
export function EmailActionPipelineCard({
  ...
  const runClassifier = async (limit = 25, force = false) => {
    ...
    try {
      ...
      toast.success(
        n > 0 ? `Found ${n} new suggestion${n === 1 ? "" : "s"}` : "No new actions found",
      );
      await load();
    } catch (e) {
      ...
      <div className="w-full min-w-0 max-w-full space-y-2 overflow-hidden">
        <div className="flex min-w-0 max-w-full items-center justify-between gap-2">
          <div className="flex items-center gap-2 min-w-0 flex-1 overflow-hidden">
            ...
            <h3 className="font-semibold text-sm truncate min-w-0 max-w-full">
              {items.length} email action{items.length === 1 ? "" : "s"}
            </h3>
          </div>
          <div className="flex items-center gap-1 shrink-0">
            ...
            <Button
              ...
              variant="ghost"
              className="h-7 w-7"
              onClick={() => runClassifier(25, false)}
              disabled={loading}
            >
        ...
        {items.map((item) => {
          ...
          return (
            <Card
              key={item.id}
              className="w-full min-w-0 max-w-full space-y-2 overflow-hidden p-3 [contain:layout]"
            >
              <div className="flex min-w-0 max-w-full items-start gap-2">
                <div className="flex-1 min-w-0 max-w-full overflow-hidden">
                  <div className="flex items-center gap-2 mb-1 flex-wrap min-w-0 max-w-full">
                    <Badge
                      variant="outline"
                      className={`text-[10px] max-w-full truncate ${catClass}`}
                    >
                  ...
                  {item.reasoning && (
                    <p className="text-[11px] text-muted-foreground line-clamp-2 [overflow-wrap:anywhere] break-words max-w-full">
                      {item.reasoning}
                    </p>
                  )}
                </div>
                <Button
                  size="sm"
                  variant="ghost"
                  className="h-7 shrink-0 px-2 text-[11px] text-muted-foreground hover:text-destructive whitespace-normal"
