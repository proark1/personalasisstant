...
function getPrayerIcon(name: string) {
  switch (name) {
    case "Fajr":
      return <Sunrise className="w-3.5 h-3.5 text-indigo-500" />;
    case "Dhuhr":
    ...
    case "Isha":
      return <MoonIcon className="w-3.5 h-3.5 text-purple-500" />;
    default:
      return null;
  }
}
...
function TimelineRow({
  ...
  return (
    <motion.div
      ...
      onClick={() =>
        onNavigate?.(
          item.type === "event" ? "calendar" : item.type === "prayer" ? "islam" : "tasks",
        )
      }
    >
      {item.type === "task" ? (
        ...
        <div className="shrink-0 ml-0.5 mr-0.5">{getPrayerIcon(item.title)}</div>
      ) : (
...
export function TodayTimeline({
  ...
  const items = useMemo(() => {
    ...
    const overdueItems: TimelineItem[] = tasks
      .filter(
        (t) => !t.trashed && !t.completed && t.dueDate && isPast(t.dueDate) && !isToday(t.dueDate),
      )
      .map((t) => ({
        id: t.id,
        title: t.title,
    ...
    const allDay = [...taskItems, ...eventItems].filter((i) => isAllDay(i));
      ...
      <GlassCardContent className="space-y-1">
        <AnimatePresence>
          {items.overdue.length > 0 && (
            <div className="mb-1">
              <span className="text-[10px] font-semibold uppercase tracking-wider text-destructive px-2.5">
                Overdue
              </span>
            </div>
          )}
          ...
          {items.allDay.map((item, i) => (
            <TimelineRow
              key={item.id}
              item={item}
              index={i + items.overdue.length + items.timed.length}
              onNavigate={onNavigate}
              onCompleteTask={onCompleteTask}
            />
          ))}
