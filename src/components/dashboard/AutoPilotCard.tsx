...
export function AutoPilotCard() {
  const { t } = useLanguage();
  const { actions, loading, running, runAutoPilot, approveAction, rejectAction, approveAll } =
    useAutoPilot();
    ...
    <Card className="glass-panel-solid">
      <CardHeader className="pb-2">
        <div className="flex items-center justify-between">
          ...
          <Button variant="ghost" size="sm" onClick={() => runAutoPilot()} disabled={running}>
            <RefreshCw className={cn("w-4 h-4", running && "animate-spin")} />
          </Button>
        </div>
        <p className="text-xs text-muted-foreground">{t("autoPilot.description")}</p>
      </CardHeader>
      <CardContent>
        {actions.length === 0 ? (
          <div className="text-center py-4">
            <Sparkles className="w-8 h-8 mx-auto mb-2 text-muted-foreground/50" />
            <p className="text-sm text-muted-foreground mb-3">{t("autoPilot.noPending")}</p>
            <Button variant="outline" size="sm" onClick={() => runAutoPilot()} disabled={running}>
          ...
          <div className="space-y-3">
            {/* Approve All Button */}
            {actions.length > 1 && (
              <Button variant="outline" size="sm" className="w-full" onClick={() => approveAll()}>
                      ...
                      {action.actionType === "reschedule_task" && action.actionData && (
                        <div className="mt-2 text-xs text-muted-foreground">
                          <span className="line-through">
                            {action.actionData.currentDueDate as string}
                          </span>
                          {" → "}
                          <span className="text-primary font-medium">
                            {action.actionData.suggestedDueDate as string}
                          </span>
                        </div>
                      )}
