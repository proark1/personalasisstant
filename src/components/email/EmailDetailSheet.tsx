...
interface EmailDetailSheetProps {
  ...
  onCreateSenderRule?: (
    pattern: string,
    rule: { default_category?: string; auto_archive?: boolean },
  ) => void;
...
const priorityOptions = [
  ...
  },
  {
    ...
    label: "Low",
    icon: ArrowDown,
    activeClass: "bg-muted text-muted-foreground border-muted-foreground/20",
    ...
    for (const e of emails) {
      if (!fullBodies[e.gmail_message_id] && !bodiesLoading.has(e.gmail_message_id)) {
        ...
        onFetchBody(e.gmail_message_id).then((body) => {
          ...
          setBodiesLoading((prev) => {
            const n = new Set(prev);
            n.delete(e.gmail_message_id);
            return n;
          });
        });
      }
        ...
        <div className="px-4 pb-8 space-y-3 overflow-y-auto max-h-[85vh]">
              ...
              <div className="flex items-center gap-1.5 flex-wrap">
                ...
                {!isThreat && onCreateSenderRule && (
                  ...
                  >
                    ...
                    Always archive {email.from_email.split("@")[1]}
                  </Button>
                )}
                <Button
                  variant="ghost"
            ...
            <GlassCard className="flex items-center gap-3 p-3 bg-primary/5 border-primary/10">
              ...
              <div className="flex-1 min-w-0">
                ...
                <div className="flex items-center gap-2 text-[10px] text-muted-foreground">
                  ...
                  {matchedContact.last_contacted_at && (
                    <span>
                      · Last contact {format(new Date(matchedContact.last_contacted_at), "MMM d")}
                    </span>
                  )}
                </div>
              </div>
          ...
          {!matchedContact && !email.matched_contact_id && onSaveAsContact && (
            <Button
              ...
              onClick={() =>
                onSaveAsContact({ name: mainSender.name || "", email: mainSender.email })
              }
            >
              <UserPlus className="w-3.5 h-3.5" />
              Save {mainSender.name || mainSender.email} as Contact
            </Button>
          )}
          ...
          <div className="flex items-center gap-3">
            <div
              className={cn(
                "w-8 h-8 rounded-full flex items-center justify-center shrink-0 text-[10px] font-semibold",
          ...
          <GlassCard variant="elevated" className="p-3 space-y-3">
            ...
            <div className="flex gap-2">
              ...
              <Button variant="outline" size="sm" onClick={copyDraft} disabled={!replyText}>
