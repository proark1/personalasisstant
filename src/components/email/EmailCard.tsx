...
export function EmailCard({
  ...
  return (
    <motion.div
      ...
      className="relative overflow-hidden rounded-xl group w-full max-w-full min-w-0 [contain:layout]"
    ...
    >
      {!selectMode && (
        <>
          <motion.div
            className="absolute inset-0 bg-emerald-500/20 flex items-center pl-4 rounded-xl"
            style={{ opacity: archiveBg }}
          >
            <Archive className="w-5 h-5 text-emerald-600" />
          </motion.div>
          <motion.div
            className="absolute inset-0 bg-amber-500/20 flex items-center justify-end pr-4 rounded-xl"
            style={{ opacity: importantBg }}
          >
      ...
      >
        ...
        <div className="flex-1 min-w-0 max-w-full overflow-hidden">
          <div className="flex min-w-0 max-w-full flex-col gap-0.5 sm:flex-row sm:items-center sm:justify-between">
            <div className="flex w-full min-w-0 items-center gap-1.5 overflow-hidden sm:flex-1">
              {email.sentiment && email.sentiment !== "neutral" && (
                <span
                  className={cn(
                    ...
                    sentimentDot[email.sentiment] || sentimentDot.neutral,
                  )}
                />
              )}
              <span
                className={cn(
                  "min-w-0 flex-1 truncate text-sm block",
                  !email.is_read
              ...
              {thread.threadCount > 1 && (
                <span className="text-[10px] font-medium bg-muted text-muted-foreground rounded-full px-1.5 py-0.5 shrink-0">
                  {thread.threadCount}
                </span>
              )}
            </div>
          ...
          <p
            className={cn(
              "mt-0.5 max-w-full text-sm whitespace-normal break-words [overflow-wrap:anywhere] line-clamp-2",
          ...
          <div className="flex items-center gap-1.5 mt-1.5 flex-wrap min-w-0 max-w-full">
            {email.ai_suggested_action && (
              <span
                className={cn(
                  "max-w-full rounded-full px-1.5 py-0.5 text-[10px] font-medium whitespace-normal break-words [overflow-wrap:anywhere]",
          ...
          <div className="flex flex-col items-center gap-1 mt-1 shrink-0">
            ...
            >
              <Star
                className={cn(
                  "w-3.5 h-3.5",
                  email.is_important ? "text-amber-500 fill-amber-500" : "text-muted-foreground",
                )}
              />
            </button>
          </div>
