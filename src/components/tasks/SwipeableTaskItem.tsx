...
export function SwipeableTaskItem({
  ...
  const deleteOpacity = useTransform(x, [-SWIPE_THRESHOLD * 1.5, -SWIPE_THRESHOLD, -20, 0], [1, 0.9, 0.3, 0]);
  const deleteScale = useTransform(x, [-SWIPE_THRESHOLD * 1.5, -SWIPE_THRESHOLD, 0], [1, 0.9, 0.5]);
  const deleteBg = useTransform(x, [-SWIPE_THRESHOLD * 1.5, -SWIPE_THRESHOLD, -40, 0], [
    'hsl(0 84% 50%)', 'hsl(0 84% 55%)', 'hsl(0 60% 65%)', 'hsl(0 60% 70%)'
  ]);
  
  // Right swipe reveals complete (green) - progressive intensity
  const completeOpacity = useTransform(x, [0, 20, SWIPE_THRESHOLD, SWIPE_THRESHOLD * 1.5], [0, 0.3, 0.9, 1]);
  const completeScale = useTransform(x, [0, SWIPE_THRESHOLD, SWIPE_THRESHOLD * 1.5], [0.5, 0.9, 1]);
  const completeBg = useTransform(x, [0, 40, SWIPE_THRESHOLD, SWIPE_THRESHOLD * 1.5], [
    'hsl(142 70% 55%)', 'hsl(142 60% 50%)', 'hsl(142 70% 45%)', 'hsl(142 76% 40%)'
  ]);

  // Text label opacity - only show near threshold
  const deleteLabelOpacity = useTransform(x, [-SWIPE_THRESHOLD * 1.2, -SWIPE_THRESHOLD * 0.7, 0], [1, 0.5, 0]);
  const completeLabelOpacity = useTransform(x, [0, SWIPE_THRESHOLD * 0.7, SWIPE_THRESHOLD * 1.2], [0, 0.5, 1]);
