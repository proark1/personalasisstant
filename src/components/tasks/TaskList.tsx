      ...
      >
        ...
        >
          ...
          >
            ...
            {!compactMode && tags.length > 0 && <TaskTagBadges tags={tags} size="sm" />}
          </div>
        </div>
        ...
        {!isSelectMode && (
          <div
            className={cn(
              ...
              compactMode ? "gap-0.5 opacity-100" : "gap-1 opacity-0 group-hover:opacity-100",
            )}
          >
            {onEditTask && (
              <button
...
export function TaskList({
  ...
  const completedTodayCount = completedTasks.filter(
    (t) => t.updatedAt && isToday(t.updatedAt),
  ).length;
  ...
  const overallProgressPercent =
    totalForProgress > 0 ? Math.round((completedTasks.length / totalForProgress) * 100) : 0;
  ...
  const handleToggleCompleteWithFeedback = useCallback(
    (id: string) => {
      vibrate("success");
    ...
    >
      ...
      <div className="flex gap-2 justify-between items-center flex-wrap">
        <div className="flex gap-2">
          <Popover open={showDueDatePicker} onOpenChange={setShowDueDatePicker}>
            <PopoverTrigger asChild>
              <Button variant="outline" size="sm" className="gap-1">
                ...
                {newTaskDueDate
                  ? format(newTaskDueDate, "MMM d", { locale: dateLocale })
                  : t("common.dueDate")}
              </Button>
            </PopoverTrigger>
            <PopoverContent className="w-auto p-0" align="start">
          ...
          {!parentId && (
            <RecurrenceSelector value={newTaskRecurrence} onChange={setNewTaskRecurrence} />
          )}
        </div>
  ...
  return (
    <div className="flex flex-col h-full">
      ...
      <div className="h-12 px-4 flex items-center justify-between border-b border-border">
        <div className="flex items-center gap-2">
          <h2 className="font-semibold text-sm">{t("nav.tasks")}</h2>
                  ...
                  >
                    <SwipeableTaskItem
                      onComplete={() => {
                        vibrate("success");
                        celebrate({
                          type: task.priority === "high" ? "highPriorityComplete" : "taskComplete",
                        });
        ...
        {completedTasks.length > 0 && (
          <div className="mt-4">
            ...
            {completedTasks.length > MAX_VISIBLE_COMPLETED && (
              ...
              >
                {showAllCompleted ? "Show less" : `Show all ${completedTasks.length} completed`}
              </Button>
            )}
          </div>
        )}
