...
export function CorrelationsDashboard() {
  const { t } = useLanguage();
  const { correlations, loading, analyzing, analyzeCorrelations, dismissCorrelation } =
    useLifeCorrelations();
  ...
  return (
    <Card className="glass-panel-solid">
      <CardHeader className="pb-2">
        ...
        <p className="text-xs text-muted-foreground">{t("correlations.description")}</p>
      </CardHeader>
      <CardContent>
        {correlations.length === 0 ? (
          <div className="text-center py-6">
            <Sparkles className="w-8 h-8 mx-auto mb-2 text-muted-foreground/50" />
            <p className="text-sm text-muted-foreground mb-3">{t("correlations.noFound")}</p>
            <Button
              variant="outline"
              size="sm"
              onClick={() => analyzeCorrelations()}
          ...
          <div className="space-y-3">
            {correlations.map((correlation) => {
              ...
              const gradientClass =
                CORRELATION_COLORS[correlation.correlationType] || "from-primary/20 to-accent/20";
