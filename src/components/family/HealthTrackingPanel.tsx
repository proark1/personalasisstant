      ...
      <Tabs value={activeTab} onValueChange={setActiveTab}>
                    ...
                    <div>
                      ...
                      {med.refill_date && (
                        <div className={`text-xs mt-2 ${
                          isPast(new Date(med.refill_date)) ? 'text-destructive' : 'text-muted-foreground'
                        }`}>
        ...
        <TabsContent value="appointments" className="mt-4">
          {appointments.length === 0 ? (
            ...
            <div className="grid gap-3">
              {appointments.filter(a => !a.is_completed).map((appt) => (
                <Card key={appt.id} className="p-4">
                  <div className="flex items-start justify-between">
                    <div>
                      <div className="flex items-center gap-2">
                        <h3 className="font-medium">{appt.title}</h3>
                        <Badge className={appointmentTypeColors[appt.appointment_type] || appointmentTypeColors.other}>
