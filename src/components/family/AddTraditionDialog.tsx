...
export function AddTraditionDialog({ open, onOpenChange }: Props) {
  ...
  const submit = async (e: React.FormEvent) => {
    ...
    if (!title) return;
    await addTradition({
      title, description: description || null, cadence,
      next_occurrence: nextOccurrence || null, is_active: true,
    });
    setTitle(''); setDescription(''); setNextOccurrence('');
    onOpenChange(false);
  };
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent>
        <DialogHeader><DialogTitle>Add Family Tradition</DialogTitle></DialogHeader>
        <form onSubmit={submit} className="space-y-3">
          <div className="space-y-2">
            <Label>Title *</Label>
            <Input value={title} onChange={e => setTitle(e.target.value)} placeholder="Friday pizza night" required />
          </div>
          <div className="space-y-2">
            <Label>Cadence</Label>
