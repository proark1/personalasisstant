...
export function FamilyHealthSafetyCard() {
  const { documents, emergencyContacts, insurance, expiringDocuments, isLoading } =
    useFamilyHealth();
              ...
              <section className="space-y-2">
                ...
                {expiringDocuments.slice(0, 3).map((d) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      {days !== null && (
                        <Badge
                          variant={days < 30 ? "destructive" : days < 90 ? "default" : "secondary"}
                        >
                ...
                {documents.length === 0 && (
                  <p className="text-xs text-muted-foreground">
                    No documents tracked. Add passports, IDs, visas to get expiry alerts.
                  </p>
                )}
              </section>
              ...
              <section className="space-y-2">
                ...
                {emergencyContacts.slice(0, 3).map((c) => (
                  ...
                  >
                    ...
                    <a href={`tel:${c.phone}`} className="text-primary text-xs">
                      {c.phone}
                    </a>
                  </div>
                ))}
                {emergencyContacts.length === 0 && (
                  <p className="text-xs text-muted-foreground">
                    Add emergency contacts for quick access in a crisis.
                  </p>
                )}
              </section>
