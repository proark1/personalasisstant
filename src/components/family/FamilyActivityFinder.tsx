...
export function FamilyActivityFinder({ onClose: _onClose }: FamilyActivityFinderProps) {
  ...
  const quickSuggestions = [
    {
      label: t("family.activities.quickRainy"),
      query: "What indoor activities can we do on a rainy day?",
    },
    {
      label: t("family.activities.quickEducational"),
      query: "Suggest educational activities that are also fun",
    },
    {
      label: t("family.activities.quickOutdoor"),
      query: "What outdoor activities can we do as a family?",
    },
    {
      label: t("family.activities.quickWeekend"),
      query: "Plan a fun family weekend with different activities",
    },
    {
      label: t("family.activities.quickQuick"),
      query: "Suggest fun activities we can do in under 30 minutes",
    },
  ];
        ...
        <div className="flex flex-wrap gap-2 text-sm text-muted-foreground">
          ...
          {children.length > 0 && (
            <div className="bg-muted px-2 py-1 rounded-full">
              {children.length}{" "}
              {children.length === 1
                ? t("family.activities.child")
                : t("family.activities.children")}
            </div>
          )}
        </div>
