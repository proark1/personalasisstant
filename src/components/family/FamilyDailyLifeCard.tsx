...
export function FamilyDailyLifeCard() {
  const { chores, allowance, mealPrefs, sleepSchedules, loading, completeChore } = useFamilyDailyLife();
  ...
  const memberName = (id: string | null) => id ? members.find(m => m.id === id)?.name || '—' : 'Anyone';
    ...
    <div className="space-y-4">
      ...
      <Card>
        ...
        <CardContent className="space-y-2">
          {chores.length === 0 && (
            <p className="text-sm text-muted-foreground">No chores yet. Add one to start tracking.</p>
          )}
          {chores.filter(c => c.is_active).map(c => (
            <div key={c.id} className="flex items-center justify-between rounded-lg border p-3">
              <div className="min-w-0">
                ...
                <div className="text-xs text-muted-foreground flex items-center gap-2">
                  ...
                  <span className="flex items-center gap-1"><Sparkles className="h-3 w-3" /> {c.points}pt</span>
                </div>
              </div>
      ...
      <Card>
        ...
        <CardContent className="space-y-2">
          {mealPrefs.length === 0 && <p className="text-sm text-muted-foreground">No preferences saved.</p>}
          {mealPrefs.map(p => (
            <div key={p.id} className="rounded-lg border p-3 text-sm">
              <div className="font-medium">{memberName(p.family_member_id)}</div>
              {!!p.loves?.length && <div className="text-xs text-muted-foreground mt-1">❤️ {p.loves.join(', ')}</div>}
              {!!p.dislikes?.length && <div className="text-xs text-muted-foreground">🚫 {p.dislikes.join(', ')}</div>}
              {!!p.dietary_restrictions?.length && <div className="text-xs text-muted-foreground">⚠️ {p.dietary_restrictions.join(', ')}</div>}
            </div>
          ))}
        </CardContent>
      </Card>
      ...
      <Card>
        ...
        <CardContent className="space-y-2">
          {sleepSchedules.length === 0 && <p className="text-sm text-muted-foreground">No schedules saved.</p>}
