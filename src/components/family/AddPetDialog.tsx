...
export function AddPetDialog({ open, onOpenChange }: Props) {
  ...
  const submit = async (e: React.FormEvent) => {
    ...
    if (!name) return;
    await addPet({
      name, species: species || null, breed: breed || null,
      vet_name: vetName || null, vet_phone: vetPhone || null,
      ...
      <DialogContent className="max-w-lg">
        ...
        <form onSubmit={submit} className="space-y-3">
          <div className="grid grid-cols-2 gap-3">
            ...
            <div className="space-y-2">
              <Label>Species</Label>
              <Input value={species} onChange={e => setSpecies(e.target.value)} placeholder="Dog, Cat…" />
            </div>
            <div className="space-y-2">
              <Label>Breed</Label>
            ...
            <div className="space-y-2">
              ...
              <Input type="date" value={nextCheckup} onChange={e => setNextCheckup(e.target.value)} />
            </div>
          </div>
          <div className="space-y-2">
            <Label>Notes</Label>
