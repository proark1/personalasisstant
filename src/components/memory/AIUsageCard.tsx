...
export function AIUsageCard() {
  ...
  const tone = s.over_cap ? "bg-destructive" : s.used_pct >= 85 ? "bg-amber-500" : "bg-primary";
    ...
    <Card className="p-3 space-y-3">
          ...
          <div className="flex items-end gap-1 h-12">
            {[...usage.monthly].reverse().map((m) => {
              ...
              return (
                ...
                >
                  <div className="w-full bg-primary/40 rounded-t-sm" style={{ height: `${h}%` }} />
                </div>
              );
            })}
          </div>
      ...
      {s.over_cap && (
        <p className="text-[10px] text-destructive">
          AI features will return early until next month — ask the operator to bump your cap if
          needed.
        </p>
      )}
    </Card>
