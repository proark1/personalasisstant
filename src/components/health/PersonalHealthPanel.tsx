...
export function PersonalHealthPanel() {
  ...
  return (
    <PanelShell
      ...
      subtitle={`${medications.filter((m) => m.is_active !== false).length} active meds · ${doctors.length} doctors`}
      loading={isLoading}
    >
      <Tabs defaultValue="medications" className="w-full">
            ...
            <Card key={m.id} className="p-3 flex justify-between">
              <div>
                ...
                <p className="text-xs text-muted-foreground">
                  {m.dose || "—"} · {m.frequency || "—"} · refill: {m.refill_date || "—"}
                </p>
              </div>
              <Button
                size="icon"
                variant="ghost"
                onClick={() => remove("personal_medications", m.id)}
              >
          ...
          {doctors.map((d) => (
            <Card key={d.id} className="p-3 flex justify-between">
              <div>
                ...
                <p className="text-xs text-muted-foreground">
                  {d.specialty || "—"} · last: {d.last_visit || "—"} · next: {d.next_visit || "—"}
                </p>
              </div>
              <Button size="icon" variant="ghost" onClick={() => remove("personal_doctors", d.id)}>
                <Trash2 className="h-4 w-4" />
              </Button>
            </Card>
          ))}
        ...
        <TabsContent value="labs" className="space-y-3 mt-4">
          <div className="flex gap-2">
            ...
            <Button
              onClick={() => {
                if (lab) {
                  addLabResult({ test_name: lab, value: labVal ? Number(labVal) : null });
                  setLab("");
                  setLabVal("");
                }
              }}
          ...
          {labResults.map((r) => (
            <Card key={r.id} className="p-3 flex justify-between">
              <div>
                ...
                <p className="text-xs text-muted-foreground">
                  {r.test_date} · {r.status || "—"}
                </p>
              </div>
              <Button size="icon" variant="ghost" onClick={() => remove("lab_results", r.id)}>
                <Trash2 className="h-4 w-4" />
              </Button>
            </Card>
          ))}
        ...
        <TabsContent value="workouts" className="space-y-3 mt-4">
          <div className="flex gap-2">
            <Input
              placeholder="Workout type (e.g. Run, Push day)"
              value={workType}
            ...
            <Button
              onClick={() => {
                if (workType) {
                  addWorkout({ workout_type: workType });
                  setWorkType("");
                }
              }}
            >
          ...
          {workouts.map((w) => (
            <Card key={w.id} className="p-3 flex justify-between">
              <div>
                ...
                <p className="text-xs text-muted-foreground">
                  {w.workout_date} · {w.duration_minutes || 0} min
                </p>
              </div>
              <Button size="icon" variant="ghost" onClick={() => remove("workouts", w.id)}>
                <Trash2 className="h-4 w-4" />
              </Button>
            </Card>
          ))}
