...
function calculateHealthScore(contract: Contract): {
  score: number;
      ...
      <Tooltip>
        <TooltipTrigger asChild>
          <div className={cn("flex items-center gap-2", className)}>
            ...
            {issues.length > 0 && <Info className="w-3.5 h-3.5 text-muted-foreground" />}
          </div>
        </TooltipTrigger>
...
export function ContractHealthBadge({ contract }: { contract: Contract }) {
  ...
  return (
    <Badge variant={variant} className="text-xs gap-1">
      {score >= 60 ? <CheckCircle className="w-3 h-3" /> : <AlertTriangle className="w-3 h-3" />}
      {score}
    </Badge>
  );
}
