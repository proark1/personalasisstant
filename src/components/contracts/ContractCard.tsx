...
export function ContractCard({
  ...
  const cancellationDate =
    contract.renewalDate && contract.autoRenews
      ? new Date(
  ...
  const isReminderSnoozed =
    contract.reminderSnoozedUntil && isFuture(contract.reminderSnoozedUntil);
      ...
      >
        <GlassCardContent className="p-4">
          <div className="flex items-start gap-3">
            ...
            {showBulkSelect && (
              <Checkbox checked={isSelected} onCheckedChange={onSelectChange} className="mt-1" />
            )}
