...
export function TeamChatPanel({ userId }: TeamChatPanelProps) {
  ...
  const renderMessage = (
    ...
    const reactions = (
      "reactions" in msg && Array.isArray(msg.reactions) ? msg.reactions : []
    ) as MessageReaction[];
    const isRead =
      "is_read" in msg ? msg.is_read : "read_by" in msg && (msg.read_by?.length || 0) > 0;

    return (
      <div
        key={msg.id}
        className={cn("flex mb-1 px-4", isOwn ? "justify-end" : "justify-start", isLast && "mb-2")}
      >
        <div
          className={cn(
  ...
  return (
    <div className="h-full flex flex-col bg-background">
      ...
      <Tabs
        value={activeTab}
        onValueChange={(v) => setActiveTab(v as "chats" | "groups")}
              ...
              {filteredConversations.length === 0 ? (
                <div className="text-center py-12 text-muted-foreground">
                  ...
                  <p className="text-xs mt-1">
                    {language === "de"
                      ? "Tippe auf ein Teammitglied um zu chatten"
                      : "Tap on a team member to start chatting"}
                  </p>
                </div>
              ) : (
                  ...
                  >
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="text-sm text-muted-foreground truncate">
                        {group.lastMessage ||
                          `${group.members?.length || 0} ${language === "de" ? "Mitglieder" : "members"}`}
                      </p>
                    </div>
                  </button>
                ))
