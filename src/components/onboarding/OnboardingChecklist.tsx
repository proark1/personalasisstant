...
export function OnboardingChecklist() {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      const [settingsRes, tgRes, calRes] = await Promise.all([
        supabase.from('proactive_settings')
          .select('onboarding_checklist_dismissed').eq('user_id', user.id).maybeSingle(),
        supabase.from('telegram_links')
          .select('id').eq('user_id', user.id).eq('is_active', true).maybeSingle(),
        supabase.from('external_calendar_connections')
  ...
  const steps = [
    ...
    { key: 'telegram',  label: 'Link Telegram so Dori can reach you', done: !!telegramLinked, href: '/settings' },
    { key: 'calendar',  label: 'Connect your calendar',            done: !!calendarLinked,  href: '/settings' },
    { key: 'workspace', label: 'Create or join a workspace',       done: hasWorkspace,      href: '/workspaces' },
  ];
  ...
  async function dismiss() {
    ...
    const { error } = await supabase.from('proactive_settings').upsert({
      user_id: user.id, onboarding_checklist_dismissed: true, updated_at: new Date().toISOString(),
    }, { onConflict: 'user_id' });
    if (error) {
  ...
  async function seedTasks() {
    if (!user?.id) return;
    const cleaned = topOfMind.map((t) => t.trim()).filter(Boolean).slice(0, 3);
      ...
      <CardContent className="space-y-3">
        <div className="space-y-1">
          {steps.map((s) => (
            ...
            >
              <span className={`w-5 h-5 rounded-full border flex items-center justify-center shrink-0 ${s.done ? 'bg-primary border-primary text-primary-foreground' : 'border-muted-foreground/30'}`}>
          ...
          <div className="rounded-md border border-border bg-background/60 p-3 space-y-2">
            ...
            <div className="space-y-1.5">
              {topOfMind.map((v, i) => (
                <Input
                  key={i}
                  placeholder={i === 0 ? 'e.g. Draft the investor update' : i === 1 ? 'e.g. Follow up with Sarah' : 'e.g. Book a dentist'}
                  value={v}
                  onChange={(e) => setTopOfMind((prev) => prev.map((p, j) => (j === i ? e.target.value : p)))}
            ...
            <div className="flex gap-2">
              ...
              <Button size="sm" variant="ghost" onClick={dismiss}>Skip for now</Button>
            </div>
          </div>
        )}
      </CardContent>
