  ...
  return (
    <motion.div variants={staggerContainer} initial="hidden" animate="show" className="space-y-3">
      ...
      <motion.div variants={staggerItem} className="flex items-center justify-between">
        <div>
          <h3 className="text-lg font-bold text-foreground">{format(today, "EEEE", { locale })}</h3>
          <p className="text-sm text-muted-foreground">
            {format(today, "d MMMM", { locale })} · {totalMeals}{" "}
            {totalMeals === 1 ? "meal" : "meals"} planned
          </p>
        </div>
      </motion.div>
              ...
              {isEmpty ? (
                ...
                <div className="space-y-2 pl-6">
                  {meals.map((meal) => {
                    ...
                    const totalTime =
                      (meal.recipe?.prep_time_minutes || 0) + (meal.recipe?.cook_time_minutes || 0);
                    ...
                    return (
                      <div
                        ...
                        onClick={() =>
                          hasRecipe && meal.recipe_id && handleViewRecipe(meal.recipe_id)
                        }
                      >
