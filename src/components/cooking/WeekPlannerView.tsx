...
const mealTypeConfig: Record<string, { icon: React.ReactNode; gradient: string; label: string; labelDe: string }> = {
  breakfast: { icon: <Sunrise className="h-3.5 w-3.5" />, gradient: 'from-amber-500/15 to-orange-500/10 border-amber-500/20', label: 'Breakfast', labelDe: 'Frühstück' },
  lunch: { icon: <Sun className="h-3.5 w-3.5" />, gradient: 'from-emerald-500/15 to-teal-500/10 border-emerald-500/20', label: 'Lunch', labelDe: 'Mittagessen' },
  dinner: { icon: <Moon className="h-3.5 w-3.5" />, gradient: 'from-indigo-500/15 to-purple-500/10 border-indigo-500/20', label: 'Dinner', labelDe: 'Abendessen' },
  snack: { icon: <Apple className="h-3.5 w-3.5" />, gradient: 'from-pink-500/15 to-rose-500/10 border-pink-500/20', label: 'Snack', labelDe: 'Snack' },
};
...
export function WeekPlannerView() {
  ...
  const getMealsForDay = (date: Date) => mealPlans.filter(m => m.meal_date === format(date, 'yyyy-MM-dd'));
  ...
  const handleGenerateShoppingList = async () => {
    const ingredients = await generateShoppingList(format(weekStart, 'yyyy-MM-dd'), format(weekEnd, 'yyyy-MM-dd'));
    if (ingredients.length === 0) {
      toast.error(language === 'de' ? 'Keine Zutaten. Planen Sie zuerst Mahlzeiten mit Rezepten!' : 'No ingredients. Plan meals with recipes first!');
      return;
    }
    const list = await addList({
      ...
      category: 'grocery', assigned_to: null, is_template: false, is_completed: false, due_date: null,
    });
    if (list) {
      let count = 0;
      for (const ing of ingredients) {
        const result = await addItem(list.id, { name: ing.name, quantity: Math.ceil(ing.quantity || 1), unit: ing.unit, category: ing.category || 'other', is_checked: false, notes: null, added_by: null }, true);
        if (result) count++;
      }
      toast.success(language === 'de' ? `Einkaufsliste mit ${count} Artikeln erstellt!` : `Shopping list created with ${count} items!`);
    }
  };
    ...
    <div className="space-y-3">
      ...
      <GlassCard className="p-3">
        <div className="flex items-center justify-between mb-3">
          ...
          <div className="text-center">
            <p className="text-sm font-bold">{format(weekStart, 'd MMM', { locale })} — {format(weekEnd, 'd MMM', { locale })}</p>
            <p className="text-xs text-muted-foreground">{mealPlans.length} {language === 'de' ? 'Mahlzeiten' : 'meals'}</p>
          </div>
      ...
      <div className="space-y-2">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <h3 className="font-semibold text-sm">
              ...
              {isDateToday(selectedDay) && (
                <Badge variant="default" className="ml-2 text-[10px] px-1.5 py-0">{language === 'de' ? 'Heute' : 'Today'}</Badge>
              )}
            </h3>
          ...
          <Button variant="ghost" size="sm" className="h-7 text-xs gap-1" onClick={() => handleAddMeal(selectedDay)}>
            ...
            <motion.div key={type} variants={staggerItem} initial="hidden" animate="show">
              <GlassCard className={cn("p-3 bg-gradient-to-r border", config.gradient)}>
                <div className="flex items-center gap-2 mb-1.5">
                  {config.icon}
                  <span className="text-xs font-semibold">{language === 'de' ? config.labelDe : config.label}</span>
                </div>
                {meals.length === 0 ? (
                  <button
                  ...
                  <div className="space-y-1.5">
                    {meals.map((meal) => (
                      <div key={meal.id} className="flex items-center justify-between bg-background/60 rounded-lg p-2.5">
                        <span className="text-sm font-medium truncate flex-1">{meal.recipe?.name || meal.custom_meal_name || 'Unnamed'}</span>
                        <Button variant="ghost" size="icon" className="h-6 w-6 shrink-0" onClick={() => handleDeleteMeal(meal.id)}>
