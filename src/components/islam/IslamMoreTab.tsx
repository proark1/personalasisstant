...
import { Compass, MapPin, RefreshCw, Calendar, BookOpen } from "lucide-react";
              ...
              <div className="absolute inset-2 rounded-full bg-gradient-to-br from-card via-background to-card shadow-2xl border border-border/50">
                    ...
                    >
                      <div
                        className={cn(
                          ...
                          i % 18 === 0
                            ? "w-0.5 h-3 bg-foreground/60 -top-[108px]"
                            : i % 6 === 0
                              ? "w-0.5 h-2 bg-foreground/40 -top-[106px]"
                ...
                {[
                  ...
                  >
                    <span
                      className={cn(
                        "absolute left-1/2 -translate-x-1/2 -top-[85px] text-sm font-semibold",
                        color,
                      )}
            ...
            <div className="mt-8 text-center space-y-3">
              ...
              {!deviceHeading && (
                <p className="text-xs text-muted-foreground max-w-[250px] mx-auto">
                  Point your device and rotate to find the Qibla direction. The 🕋 icon points
                  towards Mecca.
                </p>
              )}
            ...
            <div className="flex flex-wrap gap-2">
              {["major", "fasting", "sunnah", "remembrance", "historical"].map((type) => {
                ...
                const dotColors: Record<string, string> = {
                  major: "bg-primary",
                  fasting: "bg-emerald-500",
                  sunnah: "bg-amber-500",
                  remembrance: "bg-purple-500",
                ...
                return (
                  <Badge
                    key={type}
                    variant="outline"
                    className={cn("text-xs capitalize", colors[type])}
                  >
                  ...
                  <GlassCard key={idx} className={cn("p-4 border-l-4", typeColors[eventType])}>
                    ...
                    {event.dua && (
                      <div className="mt-3 pt-3 border-t border-border/50 bg-muted/30 -mx-4 -mb-4 p-4 rounded-b-lg">
                        ...
                        <p className="font-arabic text-lg text-right mb-2" dir="rtl">
                          {event.dua.arabic}
                        </p>
                        <p className="text-sm italic text-muted-foreground mb-1">
                          {event.dua.transliteration}
                        </p>
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="flex-1 min-w-0">
              ...
              <p className="text-xs text-muted-foreground truncate">
                {nextEvent
                  ? `Next: ${nextEvent.name} (${differenceInDays(nextEvent.date, new Date())}d)`
