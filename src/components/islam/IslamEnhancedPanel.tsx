...
const DUAS: Dua[] = [
  ...
  },
  {
    ...
    translation:
      "O Allah, by Your leave we have reached the morning, by Your leave we have reached the evening, by Your leave we live and die, and to You is the resurrection.",
  },
  // Food
  {
    id: "food-before",
  ...
  // Travel
  {
    ...
    transliteration:
      "Subhanal-lathee sakh-khara lana hatha wa ma kunna lahu muqrineen. Wa inna ila Rabbina lamunqaliboon",
    translation:
      "Glory be to Him Who has subjected this to us, and we could never have it. And to our Lord we shall return.",
  },
  {
    id: "travel-return",
    category: "Travel",
  ...
  },
  {
    ...
    transliteration:
      "Hasbiyallahu la ilaha illa Huwa, 'alayhi tawakkaltu, wa Huwa Rabbul-'Arshil-'Atheem",
    translation:
  ...
  // Forgiveness
  {
    ...
    translation:
      "I seek the forgiveness of Allah, there is no deity except Him, the Living, the Sustainer, and I repent to Him.",
  },
  {
    ...
    translation:
      "O Allah, You are my Lord, there is no deity except You. You created me and I am Your servant, and I abide by Your covenant and promise as best I can.",
  },
  // Family
  {
    id: "parents",
  ...
  },
  {
    ...
    transliteration:
      "Rabbana hab lana min azwajina wa thurriyyatina qurrata a'yunin waj'alna lil-muttaqeena imama",
    translation:
  ...
  // Health
  {
    ...
    transliteration:
      "Allahumma Rabban-nas, adhhibil-ba's, washfihi wa Antash-Shafi, la shifa'a illa shifa'uka, shifa'an la yughadiru saqama",
    translation:
      "O Allah, Lord of mankind, remove the affliction and heal, for You are the Healer. There is no healing except Your healing, a healing that leaves no disease behind.",
  },
  // Protection
  {
    id: "evil-eye",
  ...
  // Prayer
  {
    ...
    transliteration:
      "Allahumma Rabba hadhihid-da'watit-tammah, was-salatil-qa'imah, ati Muhammadanil-waseelata wal-fadeelah",
    translation:
      "O Allah, Lord of this perfect call and established prayer, grant Muhammad the intercession and virtue.",
  },
  {
    id: "in-sujood",
    category: "Prayer",
  ...
  // Patience
  {
    ...
    transliteration:
      "Inna lillahi wa inna ilayhi raji'un. Allahumma'jurni fi museebati wakhluf li khayran minha",
    translation:
      "Indeed we belong to Allah, and indeed to Him we will return. O Allah, reward me in my calamity and replace it with something better.",
  },
  // Guidance
  {
    id: "istikhara",
    ...
    transliteration:
      "Allahumma inni astakhiruka bi'ilmika, wa astaqdiruka biqudratika, wa as'aluka min fadlikal-'atheem",
    translation:
...
export function IslamEnhancedPanel() {
  const { hijriToday, islamicEvents, dhikrLogs, dhikrTypes, incrementDhikr, resetDhikr } =
    useIslamicFeatures();

  const { bookmarks, isBookmarked, addBookmark, removeBookmarkByAyah, updateBookmarkNote } =
    useQuranBookmarks();
  const { markAyahAsRead, isAyahRead, getSurahProgress, todayAyahsRead, goal } =
  ...
  const currentFontFamily =
    QURAN_FONTS.find((f) => f.id === selectedFont)?.fontFamily || "Amiri, serif";
  ...
  const handleFontChange = (fontId: string) => {
    ...
    document.documentElement.style.setProperty(
      "--quran-font",
      QURAN_FONTS.find((f) => f.id === fontId)
        ?.fontFamily.split(",")[0]
        .replace(/"/g, "") || "Amiri",
    );
  };
  ...
  const fetchTranslation = async (surahNumber: number) => {
    try {
      ...
      if (data.code === 200) {
        setTranslationAyahs(
          data.data.ayahs.map((a: { numberInSurah: number; text: string }) => ({
  ...
  const playAyahAudio = (ayah: Ayah) => {
    ...
    if (ayah.audio) {
      ...
      audio.onended = () => {
        ...
        const idx =
          selectedSurah?.ayahs.findIndex((a) => a.numberInSurah === ayah.numberInSurah) || 0;
  ...
  const currentPageAyahs =
    selectedSurah?.ayahs.slice(currentPage * AYAHS_PER_PAGE, (currentPage + 1) * AYAHS_PER_PAGE) ||
    [];

  const getTranslation = (ayahNum: number) =>
    translationAyahs.find((t) => t.numberInSurah === ayahNum)?.text;
                    ...
                    {bookmarks.length === 0 ? (
                      ...
                      bookmarks.map((bm) => (
                        <Card key={bm.id} className="p-4">
                          ...
                          <p
                            className="font-arabic text-lg text-right leading-loose mb-3"
                            dir="rtl"
                          >
                            {bm.ayah_text}
                          </p>
                            ...
                            <div className="space-y-2">
                              ...
                              <div className="flex gap-2">
                                <Button
                                  size="sm"
                                  onClick={() => {
                                    updateBookmarkNote(bm.id, noteText);
                      ...
                      return (
                        ...
                        >
                          <div className="flex items-center gap-3">
                            ...
                            <p className="text-lg" style={{ fontFamily: currentFontFamily }}>
                              {surah.name}
                            </p>
                          </div>
                        </Card>
                      ...
                      >
                        ...
                        >
                          {selectedSurah?.ayahs.map((ayah) => {
                            const ayahIsRead =
                              selectedSurah && isAyahRead(selectedSurah.number, ayah.numberInSurah);
                            return (
                              <span
                        ...
                        <div className="space-y-2 mt-4">
                          ...
                          {translationAyahs.map((t) => (
                            <p key={t.numberInSurah} className="text-sm">
                              <span className="font-medium text-primary">{t.numberInSurah}.</span>{" "}
                              {t.text}
                            </p>
                          ))}
                        </div>
                          ...
                          >
                            <div className="flex items-start gap-3">
                              <div className="flex flex-col items-center gap-1">
                                ...
                                <Button
                                  ...
                                  className={cn(
                                    "h-8 w-8",
                                    selectedSurah &&
                                      isBookmarked(selectedSurah.number, ayah.numberInSurah) &&
                                      "text-amber-500",
                                  )}
                              ...
                              <div className="flex-1">
                                {tajweedEnabled ? (
                                  <p
                                    className="text-right leading-loose tajweed-text"
                                    style={{
                  ...
                  <div className="fixed bottom-20 left-1/2 -translate-x-1/2 z-50">
                    <Card className="px-4 py-2 shadow-glow flex items-center gap-3 bg-primary text-primary-foreground rounded-full">
                      ...
                      <Button
                        variant="ghost"
                        size="icon"
                        className="h-7 w-7 rounded-full text-primary-foreground hover:text-primary-foreground/80 hover:bg-primary-foreground/10"
                        onClick={stopAudio}
                      >
                        <X className="w-4 h-4" />
                      </Button>
