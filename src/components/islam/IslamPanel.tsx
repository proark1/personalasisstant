...
export function IslamPanel() {
  ...
  const [compassPermission, setCompassPermission] = useState<"pending" | "granted" | "denied">(
    "pending",
  );
  ...
  const requestCompassPermission = async () => {
    ...
    if (
      typeof (DeviceOrientationEvent as DeviceOrientationEventStatic).requestPermission ===
      "function"
    ) {
      try {
        const permission = await (DeviceOrientationEvent as DeviceOrientationEventStatic)
          .requestPermission!();
              ...
              {nextPrayer && (
                <Card className="p-4 bg-gradient-to-br from-emerald-500/20 to-teal-500/20 border-emerald-500/30">
                  <div className="text-center">
                    ...
                    <p className="text-lg text-muted-foreground mt-1 font-arabic">
          ...
          <div className="p-4 flex flex-col items-center justify-center h-full">
            <Card className="p-6 w-full max-w-sm">
              ...
              {compassPermission === "pending" && isIOS() && (
                <Button className="w-full mb-4" onClick={requestCompassPermission}>
              ...
              <div className="relative w-64 h-64 mx-auto">
                    ...
                    return (
                      <div
                        ...
                        className={cn(
                          ...
                          isCardinal
                            ? "h-3 w-0.5 bg-amber-500"
                            : isMajor
                              ? "h-2 w-0.5 bg-amber-500/60"
                ...
                <div className="absolute inset-8 rounded-full bg-background border-2 border-muted shadow-lg">
                  {/* Cardinal directions */}
                  <div className="absolute top-2 left-1/2 -translate-x-1/2 text-lg font-bold text-red-500">
                    N
                  </div>
                  <div className="absolute bottom-2 left-1/2 -translate-x-1/2 text-lg font-bold text-muted-foreground">
                    S
                  </div>
                  <div className="absolute left-2 top-1/2 -translate-y-1/2 text-lg font-bold text-muted-foreground">
                    W
                  </div>
                  <div className="absolute right-2 top-1/2 -translate-y-1/2 text-lg font-bold text-muted-foreground">
                    E
                  </div>

                  {/* Intercardinal directions */}
                  <div className="absolute top-6 right-6 text-xs text-muted-foreground/70">NE</div>
                  <div className="absolute bottom-6 right-6 text-xs text-muted-foreground/70">
                    SE
                  </div>
                  <div className="absolute bottom-6 left-6 text-xs text-muted-foreground/70">
                    SW
                  </div>
              ...
              <div className="text-center mt-6 space-y-2">
                ...
                {deviceHeading !== null ? (
                  ...
                  <p className="text-xs text-amber-500">Compass permission denied</p>
                ) : (
              ...
              {qiblaData && (
                <div className="mt-4 pt-4 border-t border-border text-center">
                  <p className="text-xs text-muted-foreground">
                    Your location: {qiblaData.latitude.toFixed(4)}°,{" "}
                    {qiblaData.longitude.toFixed(4)}°
                  </p>
                </div>
              )}
            </Card>
                ...
                <>
                      ...
                      {currentPage === 0 && selectedSurah && (
                        <div className="text-center mb-8">
                          <div className="inline-block px-8 py-4 rounded-lg bg-gradient-to-br from-emerald-500/10 to-teal-500/10 border border-emerald-500/20">
                            ...
                            <p className="text-sm text-muted-foreground">
                              {selectedSurah.revelationType} • {selectedSurah.ayahs.length} verses
                            </p>
                          </div>
                  ...
                  <div className="px-4 py-3 border-t border-border flex items-center justify-between">
                    <Button
                      ...
                      size="sm"
                      onClick={() => {
                        setCurrentAyahIndex(Math.max(0, (currentPage - 1) * AYAHS_PER_PAGE));
                        stopAudio();
                      }}
