...
function getTimeSuggestions(stats?: DoriStats) {
  ...
  if (stats?.overdueTasks && stats.overdueTasks > 0) {
    suggestions.push({
      label: `I have ${stats.overdueTasks} overdue task${stats.overdueTasks > 1 ? "s" : ""}`,
      icon: CheckSquare,
    });
  }
...
const capabilities = [
  {
    ...
    items: ["Add tasks & events", "Track habits", "Manage projects"],
    icon: CheckSquare,
  },
  {
    category: "Life",
    items: ["Shopping lists", "Meal planning", "Contract reminders"],
    icon: ShoppingCart,
  },
  {
    category: "Knowledge",
    items: ["Web search", "News & research", "Remember things"],
    icon: BookOpen,
  },
  {
    category: "Reminders",
    items: ["Location-based", "Follow-up nudges", "Contact check-ins"],
    icon: Bell,
  },
];
          ...
          <div className="h-full flex flex-col justify-end text-center relative pb-0">
            ...
            <div className="relative z-10 flex flex-col items-center">
              <img
                src={doriFish}
                alt="Dori"
                className="w-16 h-16 object-contain mb-2 drop-shadow-lg animate-[bounce_3s_ease-in-out_infinite]"
              />
              ...
              {showCapabilities && (
                <div className="grid grid-cols-2 gap-2 mt-3 text-left max-w-xs">
                  {capabilities.map((cap) => (
                    <div
                      key={cap.category}
                      className="p-2 rounded-lg border border-border/50 bg-background/80"
                    >
