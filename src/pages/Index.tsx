...
import { useAIChat, type ToolCall } from "@/hooks/useAIChat";
...
import { requiresConfirmation } from "@/lib/actionRisk";
...
const ProfileSettingsDialog = lazy(() =>
  import("@/components/settings/ProfileSettingsDialog").then((m) => ({
...
const ShareProjectDialog = lazy(() =>
  import("@/components/projects/ShareProjectDialog").then((m) => ({
    default: m.ShareProjectDialog,
  })),
);
const MorningBriefing = lazy(() =>
  import("@/components/notifications/MorningBriefing").then((m) => ({
    default: m.MorningBriefing,
  })),
);
const WeeklyReviewDialog = lazy(() =>
  import("@/components/review/WeeklyReviewDialog").then((m) => ({ default: m.WeeklyReviewDialog })),
);

interface PendingDoriConfirmation {
  title: string;
  description: string;
  confirmLabel: string;
  cardType: ActionCardData["type"];
  cardTitle: string;
  resolve: (approved: boolean) => void;
}

function doriActionCardType(tool: ToolCall["tool"]): ActionCardData["type"] {
  if (tool.includes("task")) return "task";
  if (tool.includes("event")) return "event";
  if (tool.includes("contact")) return "contact";
  if (tool.includes("contract")) return "contract";
  if (tool.includes("project")) return "project";
  if (tool.includes("habit")) return "habit";
  if (tool.includes("note")) return "note";
  if (tool.includes("email")) return "email";
  if (tool.includes("reminder")) return "reminder";
  if (tool.includes("shopping")) return "shopping";
  return "task";
}

function describeDoriToolCall(toolCall: ToolCall): Omit<PendingDoriConfirmation, "resolve"> {
  const action = toolCall.action || "run";
  const subject =
    toolCall.task?.title ||
    toolCall.task?.id ||
    toolCall.contact?.name ||
    toolCall.contact?.query ||
    toolCall.contract?.name ||
    toolCall.contract?.query ||
    toolCall.project?.name ||
    toolCall.project?.query ||
    toolCall.habit?.name ||
    toolCall.habit?.query ||
    toolCall.note?.title ||
    toolCall.note?.query ||
    toolCall.email?.to ||
    toolCall.tool.replace(/_/g, " ");
  const prettyAction = action.replace(/_/g, " ");
  const title = `Approve Dori action: ${prettyAction}`;

  return {
    title,
    description: `Dori wants to ${prettyAction} "${subject}". This action can change or remove your data, so it will only run after your approval.`,
    confirmLabel: `Approve ${prettyAction}`,
    cardType: doriActionCardType(toolCall.tool),
    cardTitle: String(subject),
  };
}
...
const Index = () => {
  ...
  const { sharedTasks: spaceSharedTasks, sharedEvents: spaceSharedEvents } = useSpaceSharedData(
    user?.id,
  );
  ...
  // Projects system
  const { projects, addProject, updateProject, deleteProject, getProjectProgress } = useProjects(
    user?.id,
  );
  ...
  // Weekly review
  const { currentReview, createOrUpdateReview, getWeeklyStats } = useWeeklyReview(user?.id);

  // Contacts for task assignment
  const { contacts, markContacted, addContact, updateContact, deleteContact } = useContacts(
    user?.id,
  );
  ...
  const [pendingDoriConfirmation, setPendingDoriConfirmation] =
    useState<PendingDoriConfirmation | null>(null);
  ...
  const confirmDoriToolCall = useCallback((toolCall: ToolCall) => {
    if (!requiresConfirmation(toolCall.tool, toolCall.action)) {
      return Promise.resolve(true);
    }

    const description = describeDoriToolCall(toolCall);
    return new Promise<boolean>((resolve) => {
      setPendingDoriConfirmation({
        ...description,
        resolve,
      });
    });
  ...
  const resolvePendingDoriConfirmation = useCallback((approved: boolean) => {
    setPendingDoriConfirmation((pending) => {
    ...
    const handler = async (e: Event) => {
      ...
      try {
        ...
        toast({
          variant: "destructive",
          title: "Couldn't undo",
          description: "Please remove it manually.",
        });
      }
    };
  ...
  const handleSendMessage = useCallback(
    async (content: string) => {
      ...
      try {
        ...
        const activeContracts: SmartContract[] = contracts
        ...
        await streamChat({
          ...
          relevantContacts: smartPayload.relevantContacts
            ? contacts
                .filter((c) => smartPayload.relevantContacts!.some((rc) => rc.name === c.name))
                .slice(0, 10)
            : undefined,
            ...
            ? {
                members: familyMembers.map((m) => {
                  ...
                  const age = birthDate
                    ...
                    : null;
                  const activities = Array.isArray(m.activities)
                    ? (m.activities as unknown[]).map((a: unknown) => ({
                        name:
                          typeof a === "string"
                            ? a
                            : a && typeof a === "object" && "name" in a
                              ...
                              : "",
                        schedule:
                          a && typeof a === "object" && "schedule" in a
                            ? String((a as Record<string, unknown>).schedule)
                            : "",
                        location:
                          a && typeof a === "object" && "location" in a
                            ? String((a as Record<string, unknown>).location)
                            : "",
                      }))
                    : [];
                  return {
          ...
          onToolCall: async (toolCall) => {
            const approved = await confirmDoriToolCall(toolCall);
            if (!approved) {
              const summary = describeDoriToolCall(toolCall);
              collectedCards.push({
                type: summary.cardType,
                action: "Skipped",
                title: summary.cardTitle,
                status: "failed",
                details: "User approval was not granted.",
              });
              toast({
                title: "Dori action skipped",
                description: summary.cardTitle,
              });
              return;
            }
            ...
            if (toolCall.tool === "manage_task" && toolCall.task) {
              if (toolCall.action === "add") {
                ...
                if (!canCreateAiAction(user?.id)) {
                  collectedCards.push({
                    ...
                    title: taskTitleRaw,
                    status: "failed",
                    details: `Daily limit reached (${remainingAiActions(user?.id)} left). Try again tomorrow.`,
                  });
                  return;
                }
                ...
                try {
                  ...
                  const existing = tasks.find((t) => t.title.toLowerCase().trim() === taskTitle);
              ...
              try {
                const newEvent = await addEvent({
                  title: eventTitleRaw,
                  startTime:
                    evt.startTime instanceof Date
                      ? evt.startTime
                      : evt.startTime
                        ? new Date(evt.startTime)
                        : new Date(),
                  endTime:
                    evt.endTime instanceof Date
                      ? evt.endTime
                      : evt.endTime
                ...
                if (newEvent) {
                  ...
                  collectedCards.push({
                    ...
                    undo: { type: "event", id: newEvent.id },
                  });
                }
              } catch (err) {
                ...
                toast({
                  variant: "destructive",
                  title: "Couldn't schedule event",
                  description: eventTitleRaw,
                });
              } finally {
              ...
              if (newNote) {
                ...
                collectedCards.push({
                  ...
                  action: "Saved",
                  title: noteTitle,
                  undo: newNote.id ? { type: "note", id: newNote.id } : undefined,
                });
              }
              ...
              if (action === "create" && contact.name) {
                const result = await addContact({
                  ...
                  contactType: (contact.contactType === "business" ? "business" : "personal") as
                    | "personal"
                    | "business",
                  notes: contact.notes || "",
                });
                if (result) {
                  recordAiActions(user?.id, 1);
                  toast({ title: "Contact Added", description: contact.name });
                  collectedCards.push({
                    type: "contact",
                    action: "Added",
                    title: contact.name,
                    undo: (result as { id?: string })?.id
                      ? { type: "contact", id: (result as { id: string }).id }
                      : undefined,
                  });
                }
              } else if (action === "delete" && contact.query) {
                const match = contacts.find((c) =>
                  c.name.toLowerCase().includes(contact.query!.toLowerCase()),
                );
                if (match) {
                ...
                if (match) {
                  ...
                  toast({
                    ...
                    description: `${match.name} marked as contacted`,
                  });
                }
              }
              ...
              if (action === "create" && contract.name) {
                ...
                const match = contracts.find((c) =>
                  c.name.toLowerCase().includes(contract.query!.toLowerCase()),
                );
                if (match) {
              ...
              if (action === "create" && project.name) {
                ...
                const match = projects.find((p) =>
                  p.name.toLowerCase().includes(project.query!.toLowerCase()),
                );
                if (match) {
              ...
              if (action === "create" && habit.name) {
                await createHabit({
                  ...
                  icon: habit.icon || "✅",
                  frequency: (habit.frequency === "weekly" ? "weekly" : "daily") as
                    | "daily"
                    | "weekly"
                    | "custom",
                ...
                const match = todayHabits.find((h) =>
                  h.name.toLowerCase().includes(habit.query!.toLowerCase()),
                );
                if (match) {
              ...
              if (action === "create" && note) {
                ...
                const match = notes.find((n) =>
                  n.title.toLowerCase().includes(note.query!.toLowerCase()),
                );
                if (match) {
              ...
              toast({
                ...
                description: `To: ${emailData.to} — "${emailData.subject}"`,
              });
              ...
              toast({
                title: "Reminder Set",
                description: `"${message}" at ${new Date(triggerAt).toLocaleTimeString()}`,
              });
            } else if (
              toolCall.tool === "get_summary" &&
              toolCall.summaryType === "contract_costs"
            ) {
  ...
  const handleGhostCommand = useCallback(
    (command: string) => {
  ...
  const handleImportEvents = useCallback(
    async (importedEvents: CalendarEvent[]) => {
  ...
  const handleAddTask = useCallback(
    async (task: Parameters<typeof addTask>[0]) => {
      return addTask(task);
    },
    [addTask],
  );
  ...
  const handleAddEvent = useCallback(
    async (event: Parameters<typeof addEvent>[0]) => {
      // Add the event
    ...
    <CallProvider userId={user?.id || ""}>
      ...
      {mode === "standard" ? (
        ...
        <Suspense fallback={<PageFallback />}>
          <LazyGhostMode onClose={() => setMode("standard")} onCommand={handleGhostCommand} />
        </Suspense>
      )}
        ...
        <Suspense fallback={null}>
          <ShareDialog
            ...
            onShare={(email, permission) =>
              shareItem(shareDialog.type, shareDialog.id, email, permission)
            }
      ...
      <AlertDialog
        open={!!pendingDoriConfirmation}
        onOpenChange={(open) => {
          if (!open) resolvePendingDoriConfirmation(false);
        }}
      >
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>
              {pendingDoriConfirmation?.title || "Approve Dori action"}
            </AlertDialogTitle>
            <AlertDialogDescription>
              {pendingDoriConfirmation?.description ||
                "Dori wants to run an action that needs your approval."}
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
            <AlertDialogCancel onClick={() => resolvePendingDoriConfirmation(false)}>
              Cancel
            </AlertDialogCancel>
            <AlertDialogAction onClick={() => resolvePendingDoriConfirmation(true)}>
              {pendingDoriConfirmation?.confirmLabel || "Approve"}
            </AlertDialogAction>
          </AlertDialogFooter>
        </AlertDialogContent>
      </AlertDialog>
